...
이 스킬은 사용자가 `/agent`를 명시하지 않아도 상황에 맞는 에이전트로 자동 실행된다. 어느 상황에 어느 에이전트나 스킬을 쓸지는 [`../_shared/proactive-routing.md`](../_shared/proactive-routing.md)가 기준 문서다 — 여기 목록을 따로 옮겨 적지 않는다. 그 파일을 읽고 상황에 맞는 항목을 고른다.
