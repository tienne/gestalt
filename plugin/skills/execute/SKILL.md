...
**`resume`으로 이어받을 때는 다시 한다.** `resumeContext`는 `completedTaskIds`와 `nextTaskIds` 같은 진행 상태만 돌려주고 `repoRules`는 안 들고 있다. 0-3의 보관은 이 스킬 런타임의 변수라 프로세스가 바뀌면 사라진다. 다시 읽는 게 맞기도 하다 — 끊긴 사이에 레포 규칙이나 선언이 바뀌었을 수 있다. Phase 0은 파일 읽기와 도구 호출뿐이라 다시 해도 싸다.
