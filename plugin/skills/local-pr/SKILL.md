...
명령은 `gestalt pr ...`이다. 부르는 형태는 [`../_shared/cli-launcher.md`](../_shared/cli-launcher.md)의 판정을 따른다 — 게슈탈트 레포 안에서는 전역 설치가 없을 수 있어 형태가 갈린다. 한 번 확인하고 그 뒤로는 같은 형태를 쓴다.
...
게시 전에 스캔한다. 위 어투 규칙을 따라 쓴다고 해도, **코드와 diff, PR 본문에 있던 말이 코멘트에 그대로 딸려오는 자리**는 쓰는 쪽 자가점검으로 안 걸린다. `review` 스킬과 `review-loop`이 같은 자리에 같은 검사를 둔 이유다.

`body-file`로 이미 떨군 파일을 그대로 넘긴다.

```bash
gestalt humanize-scan --file /tmp/c.md --register chat
```

- **10 / 11** — 게시한다. 11이면 어투는 두고 맞춤법만 고친다.
- **0** — 스캔 결과(걸린 룰, 사례, 처방)를 보고 그 코멘트만 **한 번만** 다시 쓴다. 안 걸린 코멘트는 건드리지 않는다.
- **12** — 검사할 산문이 없다. 통과가 아니라 코멘트가 인용줄뿐이라 검사가 안 된 것이다. 자기 문장을 인용 밖에 두고 다시 쓴다.

**파일 하나에 코멘트 하나다.** 여러 개를 한 파일에 모아 담으면 인용 판정이 그 파일을 한 덩어리로 본다. 하나를 통째로 인용으로 감싸도 다른 코멘트의 산문에 묻혀 안 걸린다. `--file`을 여러 번 주면 한 번의 호출로 파일별 판정을 함께 받는다.

`--register chat`이다. 코멘트는 대화 자리라 룰북이 `대화·리뷰 S1`로 올려 둔 룰들(F-7, I-7 등)까지 봐야 한다. `doc`으로 보면 그것들이 S2로 내려가 그대로 나간다. 판정 본문(`--verdict`)도 같은 기준으로 스캔한다.

**`gestalt humanize-scan`이 없는 버전이면 건너뛰고 그 사실을 출력에 적는다.** 안 한 검사를 한 것처럼 두지 않는다.
