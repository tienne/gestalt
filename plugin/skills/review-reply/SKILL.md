...
- 게시가 끝나면 `rm -rf "$scanTmp"`로 치운다.

`--register chat`이다. 답글은 대화체 자리라 룰북이 `대화·리뷰 S1`로 올려 둔 룰들까지 봐야 한다. **스캔에서 빠지는 자리는 인라인 백틱과 언어 태그 붙은 코드펜스, 그리고 `>` 블록인용 셋이다** — 상대 코멘트를 그 셋으로 재인용하면 대조가 필요한 자리가 그대로 남는다. 큰따옴표로 감싼 평문은 안 빠지므로 원문을 가리킬 땐 백틱을 쓴다.
