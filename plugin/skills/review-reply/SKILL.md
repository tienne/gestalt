...
```bash
jq -r '.replies[] | "=== \(.id)\n\(.body)\n"' replies.json > reply-drafts.md
pnpm tsx bin/gestalt.ts humanize-scan --file reply-drafts.md --register chat
```

- **종료 코드 10** — 걸린 게 없다. 미리보기로 넘어간다.
- **종료 코드 0** — 걸렸다. 스캔 결과를 그대로 `code-review-responder`에 돌려주고 걸린 답글만 다시 쓰게 한다.
- 재작성은 **한 번만** 돈다. 두 번째도 걸리면 무엇이 남았는지 미리보기에 함께 적고 사용자가 판단하게 한다.

`--register chat`이다. 답글은 대화체 자리라 룰북이 `대화·리뷰 S1`로 올려 둔 룰들까지 봐야 한다. 상대 코멘트를 재인용한 코드블록과 인용은 스캔에서 빠지므로 대조가 필요한 자리는 그대로 남는다.
