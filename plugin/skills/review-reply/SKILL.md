...
**답글을 먼저 파일로 떨군다. 셸로 넘기지 않는다.** 본문에 한글과 백틱이 섞이고 남이 쓴 코멘트에서 온 문자열이 그대로 실릴 수 있다. `ship`과 `local-pr`이 같은 이유로 파일 쓰기 도구를 쓰라고 정해 뒀다.

```bash
# --git-common-dir 는 워크트리 여럿이 같은 자리를 가리킨다. PR 식별자로 칸을 나눈다
scanTmp="$(cd "$(git rev-parse --git-common-dir)" && pwd)/gestalt-review/<PR 번호나 로컬 PR id>"
mkdir -p "$scanTmp"
echo "$scanTmp"
```

출력된 절대 경로에 파일 쓰기 도구로 답글을 쓴다. 답글마다 `=== <id>` 한 줄과 본문, 빈 줄이다.

```bash
pnpm tsx bin/gestalt.ts humanize-scan --file "$scanTmp/reply-drafts.md" --register chat; echo "EXIT=$?"
grep -c '^=== ' "$scanTmp/reply-drafts.md"
```

판정은 **종료 코드와 개수를 함께** 본다.

- **`===` 개수가 답글 수와 다르거나 파일이 비었으면 검사 실패다.** 멈추고 무엇이 어긋났는지 알린다. **종료 코드 10을 통과로 읽지 않는다** — 빈 입력도 10을 낸다.
- **종료 코드 12는 검사할 산문이 없다는 뜻이다.** 답글이 인용줄보다 적을 때 나온다 — 통과가 아니라 검사가 안 된 것이다. 답글을 통째로 `>` 로 감싸면 이 상태가 되므로 자기 문장을 인용 밖에 두고 다시 쓴다.
- **종료 코드 10이고 개수가 맞으면** 걸린 게 없다. 미리보기로 넘어간다.
- **종료 코드 0이면** 걸렸다. 스캔 결과를 그대로 `code-review-responder`에 돌려주고 걸린 답글만 다시 쓰게 한다.
- 재작성은 **한 번만** 돈다. 두 번째도 걸리면 무엇이 남았는지 미리보기에 함께 적고 사용자가 판단하게 한다.
- 게시가 끝나면 `rm -rf "$scanTmp"`로 치운다. PR 식별자로 나눈 그 칸만 지운다 — 상위 `gestalt-review`를 통째로 지우면 다른 세션 파일까지 날아간다.
