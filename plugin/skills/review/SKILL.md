...
scanTmp="$(cd "$(git rev-parse --git-common-dir)" && pwd)/gestalt-review"
mkdir -p "$scanTmp"
echo "$scanTmp"
```
...
**스캔에서 빠지는 자리는 셋입니다.** 인라인 백틱, 언어 태그가 붙은 코드펜스, 그리고 `>` 블록인용입니다. 원문을 그대로 실어야 대조가 되는 자리라 룰이 예외로 둔 것이고 검사도 같은 기준을 씁니다. 블록인용 제외는 `chat`에서만 켜집니다 — 거기서 인용은 남이 쓴 원문입니다. **큰따옴표로 감싼 평문은 안 빠집니다.** 원문을 가리켜야 하면 백틱이나 블록인용을 씁니다.
