...
**코멘트를 먼저 파일로 떨굽니다. 셸로 넘기지 않습니다.** 본문에는 한글과 백틱이 섞이고 리뷰 대상에서 온 문자열이 그대로 실릴 수 있습니다. `ship`과 `local-pr`이 같은 이유로 파일 쓰기 도구를 쓰라고 정해 뒀고 이 자리도 같습니다.

```bash
# 자리는 git 디렉토리 아래에 잡습니다. 워킹 트리에 두면 다음 diff와 신선도 가드에 섞입니다
# --git-common-dir 는 워크트리 여럿이 같은 자리를 가리킵니다. local-pr 체크아웃이
# 동시에 여러 개 떠 있을 수 있어 PR 식별자로 칸을 나눕니다
scanTmp="$(cd "$(git rev-parse --git-common-dir)" && pwd)/gestalt-review/<PR 번호나 로컬 PR id>"
mkdir -p "$scanTmp"
echo "$scanTmp"
```

출력된 절대 경로에 파일 쓰기 도구로 코멘트를 씁니다. 코멘트마다 `=== <id>` 한 줄과 본문, 그리고 빈 줄입니다 — 어느 코멘트가 걸렸는지 되짚는 표지입니다.

```bash
pnpm tsx bin/gestalt.ts humanize-scan --file "$scanTmp/review-comments.md" --register chat; echo "EXIT=$?"
grep -c '^=== ' "$scanTmp/review-comments.md"
```

판정은 **종료 코드와 개수를 함께** 봅니다.

- **`===` 개수가 코멘트 수와 다르거나 파일이 비었으면 검사 실패입니다.** 그 자리에서 멈추고 무엇이 어긋났는지 알립니다. **종료 코드 10을 통과로 읽지 않습니다** — 빈 입력도 10을 냅니다. 파일 쓰기가 어긋났는데 조용히 통과하면 이 검사가 막으려던 상황이 그대로 재현됩니다.
- **종료 코드 12는 검사할 산문이 없다는 뜻입니다.** 코멘트가 인용줄보다 적을 때 나옵니다 — 통과가 아니라 검사가 안 된 것입니다. 코멘트를 통째로 `>` 로 감싸면 이 상태가 되므로 자기 문장을 인용 밖에 두고 다시 씁니다.
- **종료 코드 10이고 개수가 맞으면** 걸린 게 없습니다. 게시합니다.
- **종료 코드 0이면** 걸렸습니다. 스캔 결과(걸린 룰, 사례, 처방)를 **그대로 `code-review-writer`에 돌려주고 걸린 코멘트만 다시 쓰게 합니다.** 안 걸린 코멘트는 건드리지 않습니다.
- 재작성은 **한 번만** 돕니다. 두 번째도 걸리면 무엇이 남았는지 사용자에게 알리고 게시할지 묻습니다. 무한 루프에 들어가지 않습니다.
- 게시가 끝나면 `rm -rf "$scanTmp"`로 치웁니다. PR 식별자로 나눈 그 칸만 지웁니다 — 상위 `gestalt-review`를 통째로 지우면 다른 세션이 쓰는 중인 파일까지 날아갑니다.
...
**스캔에서 빠지는 자리는 인라인 백틱과 언어 태그가 붙은 코드펜스, 그리고 `>` 블록인용입니다.** 원문을 그대로 실어야 대조가 되는 자리라 룰이 예외로 둔 것이고 검사도 같은 기준을 씁니다. 블록인용 제외는 `chat`에서만 켜집니다 — 거기서 인용은 남이 쓴 원문입니다.
