...
판별은 4.7단계 진입 직전에 한 번 하고 `prTarget = "github" | "local" | "none"`으로 보관합니다 (그 앞 단계는 target이 브랜치든 커밋이든 PR이든 `git diff`만으로 동작하므로 미리 알 필요가 없습니다).
...
1. **로컬 지정** — `local` 입력이 true거나(`--local` 플래그) `target`이 로컬 PR id 형식(`gestalt pr list`에 뜨는 id)인 경우입니다. 이때는 로컬 PR을 두 수단으로 찾습니다.
   - `target`에 id가 있으면 먼저 `pnpm tsx bin/gestalt.ts pr --json show <id>`로 실제 존재를 확인합니다.
   - id가 없거나(`--local`만 준 경우) `show`가 빈 결과를 내면 `pnpm tsx bin/gestalt.ts pr --json list`로 현재 브랜치의 로컬 PR을 찾습니다.
...
3. **안 끝난 로컬 PR** — 현재 브랜치에 `status`가 `open`인 로컬 PR이 있으면 → `local`입니다.

   ```bash
   pnpm tsx bin/gestalt.ts pr --json list
   ```

   로컬 PR이 열려 있다는 건 그 코드가 아직 안 정해졌다는 뜻입니다. 그 상태로 원격에 리뷰를 게시하면 두 자리에 서로 다른 판정이 남습니다. 로컬을 먼저 닫고 원격을 봅니다.

   이 갈래로 왔으면 사용자에게 한 줄 알립니다: "현재 브랜치에 안 끝난 로컬 PR {id}가 있어서 그쪽을 먼저 봐요. 원격이면 PR 번호나 URL을 주세요."
4. `gh auth status`가 실패하거나(인증 안 됨) `git remote -v`가 비어 있으면(원격 없음) → GitHub 경로가 애초에 막혀 있습니다. `pnpm tsx bin/gestalt.ts pr --json list`로 현재 브랜치의 로컬 PR을 찾습니다. 있으면 `local`, 없으면 `none`입니다. (1번이나 3번을 거쳐 내려왔으면 이미 못 찾은 뒤이므로 `none`이 됩니다.)
...
| 로컬 지정 | 로컬 PR 조회 | GitHub 지정 | 열린 로컬 PR | gh 인증 | 원격 | GitHub PR 존재 | 결과 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 있음 | 찾음 | — | — | — | — | — | `local` (1번 — 지정이 이긴다) |
| 무관 | 못 찾음 | 있음 | — | — | — | — | `github` (2번) |
| 무관 | 못 찾음 | 없음 | 있음 | — | — | — | `local` (3번) |
| 무관 | 못 찾음 | 없음 | 없음 | 실패 | 무관 | 무관 | `none` (4번) |
| 무관 | 못 찾음 | 없음 | 없음 | 성공 | 없음 | 무관 | `none` (4번) |
| 무관 | 못 찾음 | 없음 | 없음 | 성공 | 있음 | 있음 | `github` (5번) |
| 무관 | 못 찾음 | 없음 | 없음 | 성공 | 있음 | 없음 | `none` (6번) |

읽는 법 셋을 짚어 둡니다.

- 1열이 "있음"인데 로컬 PR을 못 찾으면 지정은 힘을 잃습니다. 그 뒤로는 1열이 "없음"인 행과 같은 길을 갑니다. 잘못된 id를 주고 GitHub PR이 실재하면 `github`으로 가는 6행이 그 자리입니다.
- 4번의 "로컬 PR을 찾습니다"가 여기서는 항상 못 찾습니다. 3번이 열린 로컬 PR을 이미 걸렀고 닫힌 PR은 리뷰 대상이 아니기 때문입니다. 그래서 4번의 행이 전부 `none`입니다.
- "열린 로컬 PR"은 3번에서만 봅니다. 1번이나 2번에 걸렸으면 조회하지 않습니다.
