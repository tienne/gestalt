...
이 단계는 `/review`를 처음부터 돌린 흐름뿐 아니라, **대화 도중 "이제 PR에 코멘트 남겨줘"처럼 게시만 따로 요청**받았을 때도 진입점이 됩니다 (위 triggers의 "PR에 코멘트 남겨줘" 등). 두 경우 모두 아래 **최신 여부 확인을 먼저 통과해야** 게시할 수 있습니다.

#### 게시 전 최신 여부 확인 (옛 consensus 게시 금지)
...
- **이번 세션에 방금 리뷰를 끝냈고 그 뒤 diff 변화가 없다** → consensus가 현재 diff와 같다. 곧장 게시 진행.
