...
inputs:
  ...
  audience:
    type: string
    required: false
    description: "인라인 코멘트를 누가 읽는지. peer | junior. 사용자가 붙인 `--audience junior`나 `--junior` 플래그가 이 값으로 들어온다 (`--junior` 축약은 이 스킬 전용이다 — `explain`은 대상이 여섯이라 축약을 안 둔다). 기본값 peer — 지금까지의 코멘트가 그대로 나온다"
outputs:
  - reviewIntent
  - changeContext
  - reviewReport
...
- 플래그가 없어도 사용자가 말로 밝히면 잡습니다 ("주니어한테 설명하듯", "신입이 읽을 거라"). **읽는 사람을 가리키는 말만 신호로 봅니다** — "쉽게 써줘"처럼 대상이 안 드러난 말은 리포트를 짧게 해달라는 뜻일 수도 있어 `audience`로 읽지 않습니다. 그럴 때는 리포트를 대상에 맞춰 푸는 `/explain` 쪽이 맞는지 한 번 확인합니다.
