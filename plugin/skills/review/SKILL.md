...
**`continuity-judge`는 3단계 리뷰어의 `issues`를 입력으로 받지 않습니다** — 보는 자료는 diff 전체와 목표, PR 제목과 본문, 스펙 제약뿐입니다. 그래서 리뷰어 판정을 기다리지 않고 같은 파도에 태울 수 있습니다. 이 프롬프트에 `issues`나 `review_submit` 산출물을 끼워 넣는 변경은 이 전제와 정면으로 부딪힙니다.
