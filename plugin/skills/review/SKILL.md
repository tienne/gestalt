...
**id가 명시되면 그게 우선입니다.** 아래 순서대로 훑어 처음 걸리는 갈래를 택하고 나머지는 보지 않습니다.
...
2. `gh auth status`가 실패하거나(인증 안 됨) `git remote -v`가 비어 있으면(원격 없음) → GitHub 경로가 애초에 막혀 있습니다. `pnpm tsx bin/gestalt.ts pr --json list`로 현재 브랜치의 로컬 PR을 찾습니다. 있으면 `local`, 없으면 `none`입니다. (1번을 거쳐 내려왔으면 이미 못 찾은 뒤이므로 `none`이 됩니다.)
3. `gh pr view <target>`이 성공하면(GitHub PR이 실제로 존재) → `github`.
4. 여기까지 아무 데도 안 걸렸으면(GitHub에도 로컬에도 대응하는 PR이 없는 브랜치나 커밋 범위 리뷰) → `none`입니다. 4.7단계 전체를 건너뜁니다 — 원래 동작 그대로입니다.

**갈리는 건 본문이 정본입니다.** 아래 표는 본문 1~4번을 그대로 펼친 것뿐입니다. 표와 본문이 어긋나 보이면 본문을 따르고 표를 고칩니다.

표의 1열은 위 "로컬 지정" 여부입니다. 2열은 1번이나 2번의 조회(`show` 또는 `list`)로 로컬 PR을 실제로 찾았는지입니다. 두 열의 조합에 gh 인증, 원격, GitHub PR 존재를 곱한 32가지가 아래 11행으로 남김없이 나뉩니다.
...
- 1열이 "있음"인데 로컬 PR을 못 찾으면 지정은 힘을 잃습니다. 그 뒤로는 1열이 "없음"인 행과 같은 길을 갑니다. 잘못된 id를 주고 GitHub PR이 실재하면 `github`으로 가는 4행이 그 자리입니다.
- 1열이 "없음"이면서 gh도 살아 있고 원격도 있으면 로컬 PR을 찾았는지는 결과를 바꾸지 않습니다(마지막 두 행의 "무관"). 로컬 PR을 자동으로 집어오려면 `--local`이나 로컬 PR id로 의사를 밝혀야 합니다.
