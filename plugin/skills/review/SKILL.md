...
```bash
# 코멘트 본문을 한 파일로 모읍니다. id 구분자를 넣어야 어느 코멘트가 걸렸는지 되짚습니다
jq -r '.comments[] | "=== \(.id)\n\(.body)\n"' comments.json > review-comments.md
pnpm tsx bin/gestalt.ts humanize-scan --file review-comments.md --register chat
```

- **종료 코드 10** — 걸린 게 없습니다. 그대로 게시합니다.
- **종료 코드 0** — 걸렸습니다. 스캔 결과(걸린 룰, 사례, 처방)를 **그대로 `code-review-writer`에 돌려주고 걸린 코멘트만 다시 쓰게 합니다.** 안 걸린 코멘트는 건드리지 않습니다.
- 재작성은 **한 번만** 돕니다. 두 번째도 걸리면 무엇이 남았는지 사용자에게 알리고 게시할지 묻습니다. 무한 루프에 들어가지 않습니다.

`--register chat`입니다. 코멘트는 대화체 자리라 룰북이 `대화·리뷰 S1`로 올려 둔 룰들(F-7, B-5, I-7 등)까지 봐야 합니다. `doc` 기준으로 보면 그것들이 S2로 내려가 그대로 통과합니다.

**코드블록과 인용 안은 스캔에서 빠집니다.** 원문을 그대로 실어야 대조가 되는 자리라 룰이 예외로 둔 것입니다. 검사도 같은 기준을 씁니다.
