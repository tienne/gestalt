...
`reviewIntent`와 별개로 `audience`를 함께 잡아둡니다. 4.7단계에서 코멘트 본문을 어느 눈높이로 쓸지가 이 값으로 갈립니다.
...
- **`junior`는 GitHub PR 게시 경로에서만 걸립니다.** 로컬 PR은 `review_publish`가 `code-review-writer`를 안 거치고 합의 이슈의 `message`, `suggestion`을 그대로 옮깁니다. 그래서 **1단계에서 `prTarget`이 `local`로 판명됐고 `audience`가 `junior`면 그 자리에서 바로 알리고 확인받습니다** — 4.7단계까지 가지 않습니다. 그때는 이 스킬에서 제일 무거운 서브에이전트를 이미 태운 뒤라 사용자가 옵션이 안 먹었다는 걸 값을 다 치른 다음에 듣게 됩니다.

  ```
  로컬 PR은 합의 이슈를 그대로 옮겨서 주니어 눈높이가 적용되지 않아요. 그대로 진행할까요?
  ```

  계속하겠다고 하면 `audience`를 `peer`로 내리고 4.7단계의 writer도 `peer`로 부릅니다. **승인 없이 조용히 내리지 않습니다** — 사용자가 준 옵션이 안 먹은 것이라 결과를 보고 알아채기 어렵습니다. 조건은 플래그가 아니라 **값**입니다. 말로만 밝힌 경우에도 `audience`가 `junior`면 똑같이 걸립니다.
...
**`audience`는 이 경로에 안 걸립니다.** 이 액션이 `code-review-writer`를 안 거치기 때문입니다. 그 사실을 알리고 확인받는 자리는 여기가 아니라 0단계입니다 — `prTarget`이 `local`로 판명된 직후에 묻습니다. 여기까지 왔다는 건 그 확인을 이미 받았다는 뜻이라 다시 묻지 않습니다.
