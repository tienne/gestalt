...
#### 진입 경로 두 가지 (prTarget이 none이면 여기서 끝)
...
**`prTarget`이 `none`이면 여기서 끝냅니다.** GitHub에도 로컬에도 대응하는 PR이 없는 브랜치나 커밋 범위 리뷰가 그렇습니다. 4.7단계를 통째로 건너뛰고 결과 표시로 갑니다 — 게시할 자리가 없는데 consensus를 다시 맞춰볼 이유가 없습니다.
