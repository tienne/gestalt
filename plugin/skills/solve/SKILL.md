...
세 Phase를 도는 동안 사용자가 어디까지 왔는지 볼 자리를 만든다. 패널은 best-effort다 — 패널을 못 만들거나 못 고쳐도 인터뷰와 스펙과 실행은 그대로 간다.

**`TaskCreate`가 없는 런타임이면 Phase가 바뀔 때마다 한 줄로 대신 알린다.** 이 도구는 호스트가 주는 것이라 없는 런타임이 있다. 없다고 멈추지 않는다.
...
**Phase 1에서 만든 taskId를 그대로 쓴다.** `execute` 스킬은 자기 `TaskCreate`로 실행 패널을 따로 만들지만 여기서는 그 절을 건너뛴다 — 한 흐름인데 패널이 둘로 갈라지면 사용자가 어느 쪽을 봐야 하는지 모른다.
