...
- **PR 제목**: 레포에 규칙이 있으면 그걸 먼저 따릅니다 (`CLAUDE.md`, `CONTRIBUTING.md`). 없으면 아래를 씁니다.

  | 조건 | 꼴 | 예 |
  |---|---|---|
  | `prIntent.issueRef`가 있음 | `[티켓ID] 명사구` | `[PROJ-123] 이전 대화 목록의 세션 스토리지 이전` |
  | 없음 | `type(scope): 명사구` | `refactor(chat): 이전 대화 목록의 세션 스토리지 이전` |

  **제목은 명사로 끝냅니다.** 목록에서 한 줄씩 훑는 자리라 서술형으로 끝내면 길어지고 덜 읽힙니다. 본문은 반대로 서술체입니다 — 근거는 [`author-voice.md`](../../role-agents/_shared/references/author-voice.md)의 "제목은 개조식, 본문은 서술체" 절에 있습니다.

  명사로 끝낸다고 조사까지 빼지는 않습니다. "대화 목록 세션 스토리지 이전"처럼 조사를 다 걷으면 무엇을 무엇으로 바꿨는지가 안 읽힙니다 (ai-tell F-6).

  `issueRef`에 `PROJ-123`, `#42`, URL 등 어느 꼴이 들어와도 대괄호로 감싼 식별자만 남깁니다. URL이면 끝의 식별자를 뽑고 `#42`면 `[#42]`로 씁니다. 1단계를 건너뛰어 `issueRef`가 비어 있으면 브랜치 이름에서 티켓 꼴(`[A-Z]+-\d+`)을 찾아봅니다. 그것도 없으면 `type(scope):` 쪽으로 갑니다.
...
Agent {
  ...
  prompt: "
    ...
    - **제목은 명사로 끝나 있다. 그대로 둔다** — E-8(명사구 종결)을 제목에 적용해
      서술형으로 되돌리지 않는다. author-voice.md의 \"제목은 개조식, 본문은 서술체\"
      절이 그 자리의 근거다. 앞머리의 `[티켓ID]`나 `type(scope):`도 건드리지 않는다.
      E-8은 본문에만 적용한다
