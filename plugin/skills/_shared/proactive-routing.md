...
**`ruleSourceErrors`가 비어 있지 않으면 표로 넘어가지 않는다.** 선언이 깨지면 `ruleSources`가 빈 배열로 떨어지는데, 그걸 "선언 안 함"으로 읽으면 오타 하나에 `delegate`로 넘기라던 일이 범용 에이전트로 흘러간다. 다른 다섯 자리가 같은 이유로 멈추므로 여기도 멈추고 알린다.
