...
`scope`가 걸린 `delegate` 소스는 **넓게 잡는다.** 어느 에이전트를 부를지 고르는 자리라 건드릴 파일이 아직 없다. 트리거도 요청 문구다 — rule-sources.md가 태그 근거로 쓰지 말라고 한 바로 그 값이라 그걸로 좁히지 않는다. 자세한 건 그 문서의 "파일이 아직 안 정해진 자리" 절에 있다.
