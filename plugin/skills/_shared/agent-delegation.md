...
모델은 [`agent-model.md`](./agent-model.md)의 tier 표를 따른다. 서브에이전트를 띄울 때가 tier가 실제로 효력을 갖는 유일한 지점이다.
...
서브에이전트가 `ges_agent`에 닿지 못하면 (MCP 미연결 등) 그 사실을 그대로 보고하고 멈춘다. 페르소나 없이 임의로 수행하지 않는다 — 어투와 판정 기준이 에이전트에 있는데 그걸 못 읽었으면 결과가 규칙을 안 지킨다.

메인 세션이 대신 `ges_agent get`을 해서 직접 수행하는 폴백은 쓸 수 있다. 다만 그때는 컨텍스트 절감이 사라진다는 걸 완료 보고에 밝힌다.
