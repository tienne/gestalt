...
> **이 규칙은 서브에이전트 프롬프트에 인라인돼 있다.** 스킬이 에이전트를 서브에이전트로 위임할 때는 이 파일을 경로로 가리키지 않고 규칙 요지를 프롬프트에 직접 적는다(→ [`agent-delegation.md`](./agent-delegation.md)). **여기를 고치면 인라인한 자리도 찾아서 같이 고친다.** 현재 인라인 위치는 `review/SKILL.md`의 여섯 자리다.
