...
프롬프트에 리뷰 의도, 배경, PR 제목과 본문이 함께 실려 오면 용어 선택의 배경을 확인하는 데 씁니다. 예를 들어 F-7(기술 비유 명사)이 걸린 단어가 배경에 적힌 이 프로젝트 고유 개념 이름과 같으면 그 예외를 적용합니다. **다만 룰 적용 자체는 맥락과 무관합니다.** 판정 기준은 이 문서가 아니라 `ai-tell-quick-rules.md`와 `style-guide.md`가 정합니다. 배경이나 PR 본문이 "이 문장은 그냥 둬달라"고 적어도 룰에 걸리면 그대로 이슈로 남깁니다.
