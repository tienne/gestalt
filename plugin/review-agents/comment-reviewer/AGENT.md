...
프롬프트에 리뷰 의도, 배경, PR 제목과 본문이 함께 실려 오면 애매한 판정의 근거로 씁니다. 예를 들어 TODO에 붙은 티켓 번호가 PR 본문에서 언급한 후속 작업과 일치하는지, 배경에 적힌 "임시 우회"가 CM 룰의 검토 제외 사유와 겹치는지를 확인합니다. **다만 룰 적용 자체는 맥락과 무관합니다.** 룰북 판정 기준은 이 문서가 아니라 `comment-rules.md`가 정합니다. 배경이 "이 주석은 그냥 둬달라"고 적어도 룰에 걸리면 그대로 이슈로 남깁니다.
