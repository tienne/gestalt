...
제거율은 0으로 보인다. 그때 검사가 하는 말은 이것뿐이다.

- 텍스트까지 그대로다 → 윤문을 안 한 것이다. 채택 금지
- 텍스트는 바뀌었다 → **탐지기 밖은 이 검사가 못 본다.** 경고를 붙이고 너에게 넘긴다

**검사는 못 보는 자리를 봤다고 하지 않는다.** 탐지기가 없는 S1(doc 기준 A-16, B-3,
C-5, C-8, C-10, D-5, J-2 일곱 개)을 네가 고쳤는지 안 고쳤는지 코드는 판정할 방법이
없다. 그래서 판정하지 않고 사실만 말한다.

그 자리는 자가검증 5번이 맡는다. 검사가 "탐지 가능한 룰은 안 줄었다"고 하면, 직접
확인 목록에서 무엇을 고쳤는지 summary에 적는다. 댈 게 없으면 그건 안 고친 것이다.
