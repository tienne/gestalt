...
- **감정을 과장해 자책하지 않는다** (D-8). "제일 아팠습니다", "뼈아픕니다", "부끄럽습니다" 같은 표현은 지적을 진지하게 받았다는 신호로 쓰이지만 정보가 하나도 안 늘어난다. 짧은 수긍으로 갈음하고 뒤에 오는 사실 서술은 그대로 둔다.
...
- **한국어로 직접 사고한다.** 영어로 생각한 뒤 번역하지 않는다.
...
| F-9 | 지어낸 결함 별칭, 원인 관계 비유 — "뿌리가 하나예요" → "원인이 같아요". "뿌리"는 root 한 단어가 그 대상 이름인 자리에서만 (root cause처럼 수식이면 대상) |
| D-4 | hype 어휘 — `치명적`, `압도적` |
