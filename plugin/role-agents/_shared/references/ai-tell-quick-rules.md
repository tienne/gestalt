...
**은유를 다 지우라는 게 아니다 (F-9 오적용 주의):** 겨냥하는 건 결함을 부르려고 그 자리에서 지어낸 별칭이다. "널 포인터"를 "널 값을 가리키는 포인터"로, "데드락"을 "서로 기다리다 멈춘 상태"로 풀어 쓰면 길이만 늘고 무엇을 말하는지는 오히려 흐려진다. **가르는 기준은 업계 표준어냐 하나뿐이다.** 팀에서 몇 번 쓰였다는 것으로는 예외가 되지 않는다 — 위 [측정 근거](#측정-근거) 문단과 B-4가 "팀에서 굳은 말"을 지키는 것과는 다른 자리다. 그쪽은 팀이 오래 써온 일반 어휘를 가리킨다. F-9가 보는 건 결함을 부르려고 방금 만든 이름이다.
...
| F-9 | 결함·코드 상태를 그 자리에서 지어낸 별칭으로 명명 ("맨몸 호출", "벌거벗은 API", "한 칸 남았다") | S2 / **대화·리뷰 S1** | 무엇이 빠졌고 무엇이 틀렸는지 그대로 쓴다 ("맨몸 호출" → "접두 객체 없이 함수 이름만 적혀 있다"). 별칭은 검색이 안 되고, 스레드를 처음 읽는 사람이 못 알아보고, 답글에서 재생산돼 팀 어휘로 굳는다. **예외 기준은 업계 표준어냐 하나다** — "널 포인터", "데드락", "고아 노드"는 그대로 두고, 팀에서 몇 번 쓰였다는 건 예외 사유가 아니다(그렇게 열면 답글에 한 번 재생산된 별칭이 곧바로 빠져나간다). **F-7과 방향이 반대다** — F-7은 기술 비유를 일상 말로 되돌리고 F-9는 일상 비유를 기술 대상에 씌운 쪽을 본다. 별칭이 스스로 움직이면 D-5, 조사 없이 뭉치면 F-6도 함께 본다. 탐지기가 없어 사람이 읽고 판단한다 |
