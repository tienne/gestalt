...
**은유를 다 지우라는 게 아니다 (F-9 오적용 주의):** 겨냥하는 건 결함이나 코드 상태를 부르려고 그 자리에서 지어낸 별칭이다. "널 포인터"를 "널 값을 가리키는 포인터"로, "데드락"을 "서로 기다리다 멈춘 상태"로 풀어 쓰면 길이만 늘고 무엇을 말하는지는 오히려 흐려진다. **가르는 기준은 남이 정의해 둔 이름을 그 뜻 그대로 쓰느냐 하나뿐이다.** 명세나 교과서에 뜻이 박혀 있으면 가리킬 데가 있으니 그대로 둔다. 팀에서 몇 번 쓰였다는 것으로는 예외가 되지 않는다 — 아래 [측정 근거](#측정-근거) 문단과 B-4가 "팀에서 굳은 말"을 지키는 것과는 다른 자리다. B-4는 팀이 쓰기 시작하면 예외로 인정하는데 F-9는 그걸 예외로 안 본다. B-4가 막는 건 어색함이라 팀이 받아쓰면 풀리고 F-9가 막는 건 굳는 것 자체라 팀이 받아쓰면 그게 피해다.
...
| F-11 | 영어 관용 비유를 한국어로 직역한 명사 ("뿌리가 하나다"=root cause, "낮은 가지 열매"=low-hanging fruit, "은탄환"=silver bullet) | S2 / **대화·리뷰 S1** | 비유를 걷고 그 자리의 말을 쓴다 ("뿌리가 하나예요" → "원인이 같아요"). **가르는 기준은 비유를 걷어냈을 때 자리를 대신할 한국어 명사가 이미 있느냐다** — 뿌리는 원인으로, 은탄환은 만능 해법으로 바로 대체되니 비유가 뜻에 더하는 게 없다. 대체할 말이 없어서 비유가 뜻을 나르는 자리는 예외다. **한국어에서도 굳은 관용구는 대상이 아니다** ("발목을 잡는다", "손이 많이 간다") — 영어에 원본이 있느냐가 아니라 한국어 화자가 그 뜻으로 쓰느냐로 본다. **F-9와는 출처로 갈린다** — F-9는 그 자리에서 지어낸 별칭이고 F-11은 영어에 굳은 원본이 있는 말이다. **F-7과도 갈린다** — F-7은 기술 어휘를 일상 자리에 끌어온 쪽이고 F-11은 일상 어휘를 영어 관용구째로 들여온 쪽이다. 형태로는 못 가른다 ("뿌리 깊은", "뿌리내리다"는 정상 한국어다) — 이 경계는 뜻이라 탐지기가 없다 |
