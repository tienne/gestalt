...
**말투 감도 (중요):** 같은 명사화, 번역투라도 **대화나 리뷰 코멘트에서는 문서보다 훨씬 튄다.** 문서(칼럼·리포트) 기준 S2인 룰 상당수가 대화, 리뷰 코멘트에서 **S1로 격상**된다. 사람은 대화에서 개념을 명사 덩어리로 뭉치지 않고 동사로 풀어 말하기 때문이다. 어느 룰이 격상되는지는 심각도 칸에 `S2 / **대화·리뷰 S1**`이라고 적힌 표기 하나로만 판단한다 — 여기 목록을 따로 두면 표와 갈라진다. 전체 목록이 필요하면 자체검증 5번을 본다.
...
**한 동사에 여러 뜻을 몰지 않는다.** `close`는 한국어에서 자리마다 다른 동사로 갈린다 — 코멘트나 발표는 끝내고 이슈나 스레드는 종료하고 커넥션은 연결을 끊는다. 이걸 "닫다" 하나로 몰면 그 자리마다 조금씩 어긋난다. 파일이나 커넥션을 닫는다는 기술 용어는 그대로 두되, 글이나 말을 "닫는다"고 쓰지 않는다 — "맺는다"로 바꾸는 것도 같은 실수다. 그 자리에 맞는 동사가 따로 있다.
...
| C-14 | 수량 예고 — "두 가지", "세 가지 이유"처럼 할 말의 개수를 앞세워 문단을 엶 | S2 / **대화·리뷰 S1** | 개수를 뒤로 보내거나 뺀다("두 가지 짚을 것" → "확인이 필요한 게 2건 있습니다"). 사람은 말하기 전에 개수를 세지 않는다 — 말하다 보니 몇 개가 되는 것이다. **개수가 대상의 고정 속성이면 대상이 아니다** — 게슈탈트 원리가 실제로 다섯 개라서 "다섯 가지 원리"는 사실 서술이다. 화자가 할 말을 세어 예고하는 자리만 본다. C-9 숫자 인덱싱의 사촌인데 C-9는 표기만 보고 예고 자체는 안 본다. 이 경계는 뜻이라 탐지기가 없다 |
...
| E-8 | 관형형 + 의존명사 종결 — 서술어 없이 "짚을 것", "고려할 점", "주의 사항"으로 문장을 끊음 | S2 / **대화·리뷰 S1** | 서술어를 붙여 문장을 끝낸다("두 가지 짚을 것" → "짚어볼 게 있습니다"). 목차 항목처럼 끊으면 사람이 한 말로 안 읽힌다. **헤딩, 표 셀, 체크리스트 항목은 대상이 아니다** — 거기서는 명사구 종결이 정상 문법이다. 본문 산문과 대화만 본다 |
...
| G-4 | 화자 소거 — 의견인데 "확인이 필요합니다"처럼 화자가 없어 사실 선언으로 읽힘 | S2 / **대화·리뷰 S1** | 누가 하는 말인지 살린다("확인이 필요합니다" → "제가 봤을 때는 확인이 필요합니다"). **G-2와 방향이 반대라 경계를 지킨다** — 사실 서술에 붙은 추정 어미("~로 보인다")는 G-2로 빼고 판단이나 제안에서 빠진 화자는 여기서 넣는다. 부재를 봐야 해서 탐지기가 없다 |
...
| I-5 | 사무투 분류사 "~ 건/해당 건/이번 건/그 건", 코드·문서 용어를 대화에 그대로("주석 건") | S2 / **대화·리뷰 S1** | 구체 명사로 풀기("Copilot이 짚은 주석 건" → "Copilot이 남긴 코멘트"). "건"은 공문서투 분류사라 대화에서 어색하다. **다만 수량 자리는 세는 대상을 보고 가른다** — 사물(버그·코멘트·티켓)을 셀 때만 "개"로 바꾸고("버그 8건" → "버그 8개") 처리할 안건을 셀 때는 "건"이 맞다("확인이 필요한 게 2건 있습니다"). 판정법은 그 자리에 "안건"을 넣어보는 것이다 — "확인이 필요한 안건 2개"는 말이 되고 "버그라는 안건 8개"는 안 된다. 탐지기는 이 경계를 못 가르므로 수량 자리를 안 본다 |
