...
| A-20 | "명시적으로"가 뒤 동사와 뜻이 겹쳐 잉여로 남음 (explicitly 직역 — "명시적으로 선언한다", "명시적으로 지정한다") | S2 | 부사를 뺀다 ("선언한다", "타입을 지정한다"). 선언과 지정은 그 자체가 명시하는 행위라 부사가 놀고 있다. **암묵 경로와 대비하는 자리는 예외다** — "추론에 맡기지 말고 명시적으로 선언하자", "암묵 변환 말고 명시적으로 캐스팅"은 반대편이 문장 안에 있어서 부사가 일을 한다. 이 경계는 뜻이라 탐지기가 없다 |
...
| F-10 | 코드나 스펙에 하는 행위를 일상 관용구 은유로 부름 ("타입을 못 박다", "스펙을 못박아주다") | S2 / **대화·리뷰 S1** | 그 자리에서 코드가 실제로 무엇이 되는지를 동사로 쓴다 ("타입을 선언한다", "값을 고정한다", "상수로 뺀다"). 관용구는 원래 사람 사이 합의에 걸리는 말이라, 목적어가 타입이나 값이 되면 누구에게 못을 박는다는 건지 가리킬 데가 없어진다. "-아주다"까지 붙으면 확정을 요구하는 톤이 얹혀 부탁조로도 안 읽힌다. **예외는 실제로 사람 사이 합의를 가리킬 때다** — "일정을 못 박았다", "범위를 못 박아두죠"는 상대가 번복할 여지를 막는 자리라 그대로 둔다. 값을 하드코딩한다는 뜻으로 굳은 "박아두다"도 예외다. **F-9와 보는 자리가 다르다** — F-9는 결함에 새 이름을 붙인 명사 쪽이고 F-10은 행위를 부르는 동사 쪽이다. 이 경계는 뜻이라 탐지기가 없다 |
