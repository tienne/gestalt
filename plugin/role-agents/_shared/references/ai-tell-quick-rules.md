...
**은유를 다 지우라는 게 아니다 (F-9 오적용 주의):** 겨냥하는 건 결함이나 코드 상태, 그 결함의 원인 관계를 뭉개 부른 말이다. 그 자리에서 지어낸 별칭이든 이미 있는 비유든 같다. "널 포인터"를 "널 값을 가리키는 포인터"로, "데드락"을 "서로 기다리다 멈춘 상태"로 풀어 쓰면 길이만 늘고 무엇을 말하는지는 오히려 흐려진다. **가르는 기준은 그 자리에서 무엇을 가리키는지가 말에 남아 있느냐 하나뿐이다.** 남이 정의해 둔 이름을 그 뜻 그대로 쓰면 그 기준을 통과한다. 그게 대표 사례다. **"뿌리"는 root 한 단어가 그 대상 이름인 자리에서만 남긴다** — 이를테면 트리와 스레드의 root, 디렉토리와 프로젝트와 git ref의 root다. **root cause처럼 root가 다른 명사를 수식하는 자리는 대상이다** — 원인이면 "원인", 출처면 "출처"로 쓴다. **이 기준이 아래 일반 예외보다 앞선다** — root cause analysis가 문헌에 뜻이 박힌 말이어도 "뿌리"로 옮기면 걸린다. **사전에 비유 뜻풀이가 실렸다는 것만으로는 정의해 둔 이름이 아니다** — "뿌리가 하나다"는 사전에 근본 뜻이 실려도 무엇이 같은지가 안 남는다. 명세나 교과서에 뜻이 박혀 있으면 가리킬 데가 있으니 그대로 둔다. 팀에서 몇 번 쓰였다는 것으로는 예외가 되지 않는다 — 아래 [측정 근거](#측정-근거) 문단과 B-4가 "팀에서 굳은 말"을 지키는 것과는 다른 자리다. B-4는 팀이 쓰기 시작하면 예외로 인정하는데 F-9는 그걸 예외로 안 본다. B-4가 막는 건 어색함이라 팀이 받아쓰면 풀리고 F-9가 막는 건 굳는 것 자체라 팀이 받아쓰면 그게 피해다.

**근거가 붙은 룰 (→ 아래 [측정 근거](#측정-근거)):** A-2, I-1, C-8은 대조 코퍼스 측정으로 심각도를 조정했고 B-4는 팀이 실제로 쓰는 말이라 예시에서 뺐다. C-5, C-8, C-10, F-4는 탐지기를 안 붙인 이유가 거기 있다. 원어민이 오히려 더 쓰는 표현이나 팀에서 이미 굳은 말을 지우면 사람 글을 AI 글로 만든다.
...
| A-20 | "명시적으로"가 뒤 동사와 뜻이 겹쳐 잉여로 남음 (explicitly 직역 — "명시적으로 선언한다", "명시적으로 지정한다") | S2 | 부사를 뺀다 ("선언한다", "지정한다"). 선언과 지정은 그 자체가 명시하는 행위라 부사에 남는 뜻이 없다. **암묵 경로와 대비하는 자리는 예외다** — "추론에 맡기지 말고 명시적으로 선언하자"처럼 반대편이 같은 문단이나 바로 앞뒤 문장에 있으면 남긴다. **대화·리뷰에서도 S2로 둔다** — 기술 문서는 암묵 경로와 짝지어 쓰는 자리가 많아 A-2처럼 격상하면 정당한 쓰임이 함께 걸린다. 이 경계는 뜻이라 탐지기가 없다 |
...
**탐지기는 대상이 앞에 오는 자리만 본다** — 이슈, PR, 스레드, 티켓, 논의, 리뷰, 승인, 판정, 코멘트, 발표, 라운드, 대화, 이야기, 글이 그 목록이다. 어간까지만 열어 어미는 안 가리므로 닫는다, 닫혔다, 닫아버리면이 전부 걸린다. 목록에서 문서와 세션, 파일을 뺀 건 에디터나 DB 연결처럼 실제로 닫는 대상과 겹쳐서다 — 그 자리는 표에만 두고 모델이 본다. 어순으로 빠지는 "닫힌 PR"과 "닫힌 집합"은 대상을 요구하는 덕에 자연히 안 걸린다. 다만 "닫힌 집합"은 수학 `closed set`을 옮긴 말이라 그 자리에서만 정착어다 — 개수가 정해진 목록이라는 뜻으로 쓰는 자리는 그 말로 풀어 쓴다.
...
| ID | 패턴 | 심각도 | 처방 |
|---|---|---|---|
| C-5 | 이모지 남발 | S1 | 장르 칼럼·리포트면 전부 삭제 (탐지기가 없는 이유는 아래 [측정 근거](#측정-근거)) |
| C-7 | "먼저·반면·결국" 3단 공식 | S2 | 접속사 1~2개로 줄이거나 본문에 녹여 제거 |
| C-8 | "A인가, B인가" / "A가 아니라 B" 대구 반복 | S1 | 한 번만 살리고 나머지는 비대칭 평서문·직접 단언으로. 대조 코퍼스에서 인간 대비 9.2배(개인 글 기준 18배)로 나온 최강 신호이고 모델 계열을 안 가린다 (탐지기가 없는 이유는 아래 [측정 근거](#측정-근거)) |
| C-9 | 숫자 괄호 인덱싱 "(1)·(2)·(3)" | S2 | 본문에 녹이거나 단순 줄바꿈 |
| C-10 | 콜론 부제 헤딩 "X: Y" 반복 | S1 | 콜론 뒤를 버리고 앞부분만 남긴다. **더 좋은 부제로 바꾸지 않는다** — 부제를 갈아끼우면 패턴이 그대로 남는다. 앞부분만으로 뜻이 안 서면 평서 헤딩 한 문장으로. **단 콜론 앞이 구조 라벨이면 대상이 아니다** — 커밋과 PR 제목의 `type(scope):` 접두와 리뷰 코멘트의 `r:`/`c:`/`a:` 접두, **이 두 형태만** 예외다. 팀이 형식으로 정한 문법이라 라벨은 그대로 두고 콜론 뒤 본문만 다듬는다. 열거에 없는 "X: Y"는 라벨처럼 보여도 예외가 아니다 — "요약: 세 가지"는 그냥 콜론 부제다 (탐지기가 없는 이유는 아래 [측정 근거](#측정-근거)) |
...
| E-8 | 관형형 + 의존명사 종결 — 서술어 없이 "짚을 것", "고려할 점", "주의 사항"으로 문장을 끊음 | S2 / **대화·리뷰 S1** | 서술어를 붙여 문장을 끝낸다("두 가지 짚을 것" → "짚어볼 게 있습니다"). 목차 항목처럼 끊으면 사람이 한 말로 안 읽힌다. **헤딩, 표 셀, 체크리스트 항목, 문단을 여는 볼드 도입구는 대상이 아니다** — 거기서는 명사구 종결이 정상 문법이다. **PR 제목과 커밋 제목도 대상이 아니다** — 목록에서 한 줄로 훑는 자리라 개조식이 기본이고, 이 레포는 오히려 서술형 종결을 안 쓴다 (`author-voice.md`의 "제목은 개조식, 본문은 서술체" 절). 본문은 그대로 이 룰을 받는다. **볼드 예외는 규범, 참조 문서에서 문단 안 소제목 역할을 하는 자리에만 준다** — 리뷰 코멘트와 대화에서는 볼드를 씌워도 본문 산문으로 본다. 서식으로만 예외를 주면 "두 가지 짚을 것"을 볼드 처리해 그대로 통과시키게 된다. 본문 산문과 대화만 본다. 예외가 전부 문맥이라 줄 단위 정규식으로는 못 가르고, 그래서 탐지기가 없다. G-4와 겹치면 서술어를 붙이되 판단이면 화자도 함께 남긴다 |

## F. 과도한 수식, 중복

이 절은 축이 둘이다 — F-4~F-6은 명사화이고 F-7~F-10은 비유 오용이다. 새 룰을 넣을 때 어느 축인지 먼저 정한다.

| ID | 패턴 | 심각도 | 처방 |
|---|---|---|---|
| F-4 | 한자어 명사화 -성/-적/-화 + 영어 명사화 -tion/-ment/-ness/-ity 누적 (한 글 12회+) | S2 / **대화·리뷰 S1** | 동사·형용사 어근으로 환원("the implementation of the policy" → "정책 시행" 또는 "정책을 시행하기") (탐지기가 없는 이유는 아래 [측정 근거](#측정-근거)) |
...
| F-9 | 결함이나 코드 상태, 그 결함의 원인 관계를 비유 명사로 뭉개 부름 ("맨몸 호출", "벌거벗은 API", "뿌리가 하나다") | S2 / **대화·리뷰 S1** | 무엇이 빠졌고 무엇이 틀렸는지 그대로 쓴다 ("맨몸 호출" → "접두 객체 없이 함수 이름만 적혀 있다"). 별칭은 검색이 안 되고 처음 읽는 사람이 못 알아보고 답글에서 재생산돼 팀 어휘로 굳는다. **예외는 남이 정의해 둔 이름을 그 뜻 그대로 쓸 때다** — "널 포인터", "데드락", "고아 노드"처럼 명세나 교과서에 뜻이 박힌 말이고 F-6으로도 안 넘긴다. 다른 결함에 비유로 얹으면 예외가 아니고 팀에서 몇 번 쓰였다는 것도 아니다 — 그렇게 열면 답글에 한 번 재생산된 별칭이 곧바로 빠져나간다. **F-7과 방향이 반대다** — F-7은 기술 비유를 일상 말로 되돌리고 F-9는 일상 비유를 기술 대상에 씌운 쪽을 본다. **원인 관계를 부르는 비유도 같은 자리다** — "뿌리가 하나다"는 원인이 같다는 말을 뿌리에 얹은 것이라 무엇이 같은지가 안 읽힌다. "원인이 같다"로 쓴다. 지어낸 별칭이든 이미 있는 비유든 가르는 건 하나다 — 그 자리에서 무엇을 가리키는지가 말에 남아 있느냐다. **"뿌리"는 root 한 단어가 그 대상 이름인 자리에서만 쓴다** — root cause처럼 수식하는 자리는 대상이다. 기준과 예시는 위 `F-9 오적용 주의` 문단에 있고 상세는 그 한 자리에만 둔다. F-9에 걸린 별칭이 스스로 움직이면 D-5, 조사 없이 뭉치면 F-6도 함께 걸린다. **별칭과 원인 쪽 경계는 뜻이라 탐지기가 없다.** "뿌리"는 말 하나라 F-10의 "못 박" 탐지기처럼 형태로 걸 수 있지만 안 붙였다 — 레포에 root를 가리키는 정상 용례가 서른 자리 넘게 있어 걸린 자리를 사람이 다시 보는 양이 검사로 얻는 것보다 크다 |
...
5. **잔존 S1 패턴 0건** (탐지기 수치는 각 룰의 대상 기준으로 걸러 센다 — 룰이 예외로 둔 자리는 잔존 건수에 안 넣는다): A-1, A-3, A-7, A-8, A-16, B-3, B-5, C-5, C-8, C-10, C-11, C-12, D-1~D-6, D-8, H-1, H-3, J-2가 남아있지 않은가 (대화·리뷰 말투면 A-2, A-5, C-14, D-9, E-8, F-4, F-5, F-6, F-7, F-8, F-9, F-10, G-4, I-1, I-5, I-6, I-7, I-8도 S1로 포함)
...
**탐지기를 안 붙이기로 한 것도 같다** — 근거가 없으면 다음 사람이 같은 프로토타입을 다시 만든다.
...
### 탐지기를 안 붙인 이유 (C-5, C-8, C-10, F-4)

심각도가 아니라 **코드가 이 룰을 재느냐**를 남긴다. 이 넷은 형태만 보면 셀 수 있어 보여서
탐지기 후보로 몇 번 올라온다. 붙여서 레포 마크다운 87개에 돌려본 결과가 아래다.

| 룰 | 돌려본 결과 | 왜 못 붙이나 |
|---|---|---|
| F-4 명사화 누적 | 87개 중 48개 파일이 임계값을 넘었다 | "한 글에 12회+"가 절대값이라 긴 문서는 내용과 무관하게 넘는다. 길이에 비례하는 값이 아니면 판정이 글자 수를 재는 것이 된다 |
| C-5 이모지 | 23개 파일 | 스캔은 그 파일이 어느 장르인지 모른다. 규범 문서가 규칙 표기로 쓰는 마커와 산문에 뿌린 이모지가 형태로 같다. 이 룰은 처방부터 "칼럼이나 리포트면"이라는 조건을 달고 있다 |
| C-10 콜론 헤딩 | 12개 파일 | 걸린 자리가 대부분 `### 4.5단계: 리포트 워싱` 같은 번호 라벨이었다. 예외로 열거한 것은 `type(scope):`와 `r:`/`c:`/`a:` 둘뿐이라 나머지 라벨이 전부 걸린다 |
| C-8 부정 대구 | 0개 파일 | 오탐은 안 났는데 레포에 표본이 없어 실제로 무엇을 잡는지 확인할 방법이 없었다. 위 대조 코퍼스가 심각도를 올린 근거는 AI 산문이고 이 레포 문서가 아니다 |

**임계값을 비율로 바꾸면 F-4는 열린다.** 다만 그 비율을 정하려면 측정이 따로 필요하다.
감으로 정한 비율은 감으로 정한 절대값과 다를 게 없다.

나머지 비탐지 S1은 이 표에 없다. 돌려본 적이 없어서다 — 안 붙인 게 아니라 아직 안 본 것이라,
여기 적으면 검증한 것처럼 읽힌다.
...
> v2.0 신규/보강은 A-7, A-15, A-16, A-18, A-19, E-2, E-7, F-4 **8개 (A-17 보류)**. **v2.0 이후 추가: A-20, F-10.** 룰을 더하면 이 줄에 ID만 덧붙인다 — 계기와 근거는 각 룰 본문이 갖는다. 그 사이 회차에 들어온 룰은 이 줄이 비어 있던 동안이라 git 이력에서 본다. 학술 인용 전문은 `references/scholarship.md`. post-editese 3축 지표는 본 룰북 미반영(metric only 트랙). A-17 무정물, 추상명사 '-들'은 학술 근거(전영철 2007·곽은주·진실로 2011) 강하나 외부 회차(2026-05-07 위키 6편)에서 양성 0개 — NMT 원본 출력 회차 후 v2.1에서 동일 ID로 재평가.
