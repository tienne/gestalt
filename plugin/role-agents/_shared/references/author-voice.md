...
| c: 여섯 건은 매끄럽게 다듬는 수준이고 | c: 여섯 개는 가볍게 손보면 되는 것들이에요 | 형식명사 "수준"으로 정도 뭉개기 + 코멘트는 사물이라 "개"(I-5) |
...
"짚어볼 게 있습니다", "확인이 필요합니다"처럼 서술어를 붙인다. 헤딩이나 체크리스트 항목은
명사구 종결이 정상 문법이라 대상이 아니다 (ai-tell E-8).
...
- 셀 때는 대상을 보고 고른다. 사물(코멘트·버그·티켓)은 **"8개"**, 처리할 안건은 **"2건"** (ai-tell I-5)
...
> | "c: 여섯 **건**은 다듬는 정도예요" — 코멘트는 사물이다 | "제가 놓친 **건** 맞아요" — "것은"의 준말 |
> | "버그 8**건** 고쳤어요" — 버그도 사물이다 | "확인이 필요한 게 2**건** 있습니다" — 안건을 센다 |
>
> 준말 "건"은 구어에서 자연스러운 표현이라 늘려 쓰면("안 돌려본 것은") 오히려 딱딱해진다.
> 수량을 세는 자리는 **세는 사물이 명사로 드러났는지**로 가른다. 드러났으면 "개"다
> ("코멘트 여섯 개", "버그 8개", "티켓 8개"). 명사 없이 처리할 일 자체를 셀 때만 "건"이다
> ("확인이 필요한 게 2건"). 앞에 명사가 붙으면 그게 기준이라 "티켓이 8건"은 "8개"로 간다.
> 이 기준으로 위 표의 두 자리가 같은 답을 낸다. 코멘트는 명사로 드러났으니 "여섯 개"다.
> "확인이 필요한 게"는 명사가 없으니 "2건"이다.
