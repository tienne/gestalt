...
**첫째 줄은 룰 ID 사본을 본다.** 에이전트 문서 14곳이 룰 ID와 금지 어휘를 손으로 옮겨 적고 있어서 룰북에서 ID를 지우거나
...
`HOIST`에 적힌 룰마다 그 조각을 본다. 이것도 `pnpm test`와 `pnpm build`(postbuild)에
...
- **탐지기를 안 붙이기로 한 것도 같은 자리에 남긴다.** C-5, C-8, C-10, F-4가 그렇다. 형태로 셀 수
  있어 보이는 룰이라 후보로 다시 올라오는데, 안 붙인 근거가 없으면 다음 사람이 같은 프로토타입을
  또 만든다.
