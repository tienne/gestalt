...
보고서 전용 문체와 프레이밍: `references/report-register.md` — 진행현황, 성과 분석, 제안서, 의사결정 메모처럼 기록성과 정밀도가 필요한 문서에만 적용한다.
...
5. **반론을 먼저 다룬다.** 제안서, RFC는 읽는 사람이 떠올릴 반대 의견(비용, 리스크, 대안)을 작성자가 먼저 꺼내 답한다. 리스크는 대응과 함께 적는다 — 대응을 적을 수 없는 리스크는 뺀다. 대응 없는 나열은 문제 제기로 읽혀 신뢰를 깎는다.
...
9. **절 제목은 명사구로.** 서술형 제목("매출이 급증했다")은 논평처럼 읽히고 문서를 갱신할 때 낡는다. 제목은 주제를 가리키고 주장은 본문이 맡는다 — 표 항목 라벨과 자유 서술 절 제목 모두 해당한다. 발표 슬라이드 제목(주장·결론형이 맞는 자리, style-guide.md 참조)과는 반대 방향이니 섞지 않는다.
10. **보고서 문체는 별도 패스로 다룬다.** 진행현황, 성과 분석, 제안서, 의사결정 메모는 초안 뒤 `report-register.md`로 의미역, 상태 표기, 절 프레이밍을 검토한다. 이 단계는 구조를 다룰 수 있지만 뒤이은 humanize는 표현만 다듬는다.
