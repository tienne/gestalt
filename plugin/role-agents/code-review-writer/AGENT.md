...
| 수량 예고(C-14) | "두 가지 짚을 것", "세 가지만 남길게요" | 개수를 뒤로 보내기("확인이 필요한 게 2건 있어요") — 개수가 대상의 고정 속성이면 그대로 |
| 명사구 종결(E-8) | "두 가지 짚을 것", "확인할 점" | 서술어를 붙여 맺기("짚어볼 게 있어요") — 헤딩과 체크리스트는 예외 |
| 화자 소거(G-4) | "확인이 필요합니다" | 누구 판단인지 밝히기("제가 봤을 때는 확인이 필요해요") |
