...
`junior`면 [`../explainer/references/audience.md`](../explainer/references/audience.md)의
`junior` 항목을 읽고 그 기준을 코멘트에 적용한다. 그 대상표는 설명글 기준이라 리뷰 코멘트에
그대로 겹치지 않는 자리가 있다. 겹치는 것과 안 겹치는 것을 아래에서 가른다.
...
- **전문용어는 첫 등장에 한 줄 정의를 붙이고 그다음부터 그냥 쓴다.** 리뷰 하나가 코멘트
  ...
  풀면 읽는 사람이 무시하기 시작한다.
- **왜 이 방법이냐를 한 줄 더 적는다.** 대안을 하나 언급하고 왜 안 골랐는지까지. 주니어에게는
  고칠 자리보다 고르는 법이 남는다.
...
- 개행 규칙, 출처 태그 금지, 내부 에이전트 이름 감추기.
- 어투. 대상표의 `junior`는 해요체에 제안형이라 이미 쓰던 voice와 같다. 새로 만들지 않는다.

**경계 하나.** 정의를 붙인다고 코멘트가 강의가 되면 안 된다. 정의는 한 줄이다. 그 줄이
빠져도 무엇을 고쳐야 하는지가 읽혀야 한다. 주니어에게 제일 나쁜 리뷰는 어려운 리뷰가 아니라
길어서 안 읽는 리뷰다.

`nontech`, `manager`, `exec`, `outsider`는 이 자리에서 안 받는다. 용어를 전면 금지하는
대상은 인라인 코멘트와 안 맞는다 — `path`, `line`에 붙어 수정 스니펫을 주는 게 이 코멘트가
하는 일이라, 용어와 코드를 걷어내면 리뷰이가 무엇을 고쳐야 할지 못 읽는다. 그 대상에게는
리뷰가 끝난 뒤 리포트를 `explainer`에게 넘긴다.
