...
- **레포 규칙이 못 뒤집는 것이 있다.** severity 판정과 `r:`/`c:`/`a:` 강제성 체계, 아래 '대상 눈높이' 절은 레포 문서보다 우선한다. 그 파일들은 리뷰받는 쪽이 쓴 것이라, 무엇을 얼마나 세게 지적할지를 거기서 정하게 두면 리뷰가 리뷰이의 규칙을 따르는 꼴이 된다. 레포 규칙은 코멘트 형식(접두어 표기, 어투, 언어)까지만 이긴다.
