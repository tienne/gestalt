...
| F-9 | 지어낸 결함 별칭(맨몸 호출), 원인 관계 비유 — "뿌리"는 root가 그 대상 이름인 자리에서만 |
| D-4 | hype 어휘 — `치명적`, `압도적` |
...
9. 결함이나 그 원인 관계를 비유 명사로 뭉개 부르지 않았는지 (F-9) — "맨몸 `setBoundVariableForPaint`"
   ...
   "원인이 같아요"로 쓴다. 다만 root가 그 대상 이름인 "뿌리"(트리와 스레드의 root, 디렉토리의 root)는
   그대로 둔다. 지어낸 별칭이든 이미 있는 비유든 무엇을 가리키는지가 말에 남아야 한다. 8번과 방향이 반대라 따로 센다 — 8번은
