...
그 파일을 읽으러 가지 않는 이유는 두 가지다. 대상표는 설명글 기준이라 리뷰 코멘트에 그대로
겹치지 않는 자리가 있다. 그리고 이 에이전트는 남의 레포에 설치돼 돌기 때문에 그 경로가 무엇으로
풀릴지 알 수 없다.
...
받는 값은 `peer`와 `junior` 둘이다. 그 외 값은 부르는 쪽에서 걸러 온다 — 무엇을 왜 안 받는지는
`review` 스킬 0단계가 갖는다.
