...
| 대입 | ~라고 생각하면, ~고 생각해보~, ~고 생각해 보~, ~인 셈, ~같은 거, ~같은 것 |
| 그려보기 | 떠올려보~, 떠올려 보~, 상상해보~, 상상해 보~ |
| 유사 | 비슷, ~처럼, ~과 같이 |

`~`는 그 자리에 다른 말이 와도 걸린다는 뜻이다. `~고 생각해보~`는 "적는다고 생각해보세요"와
"우편함이라고 생각해보면"을 다 잡는다. 앞의 `~고`를 빼고 "생각해보"만 찾으면 "곰곰이
생각해보니"처럼 비유가 아닌 문장도 통과하므로 연결어미까지 함께 본다.
