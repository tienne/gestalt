...
끈 자리를 무엇이 대신 보느냐가 남는데, 그건 원문과의 연결이다. 용어가 아니라 원문이 붙들고
있던 한글 내용어를 하나라도 담았는지를 `explain-check`가 따로 잰다. 룰북이 금지한 건 전문용어지
내용이 아니라 이 검사는 어느 대상에게나 걸린다. 용어를 허용한 `junior`와 `peer`, `manager`는
거기에 더해 핵심어를 몇 개 남겼는지도 잰다.

그 연결 검사는 바닥이지 저울이 아니다. 좋은 설명일수록 원문 어휘를 버리므로 하나만 요구한다.
원문에 한글 내용어가 넷도 안 되면(영문 스택 트레이스가 그렇다) 잴 근거가 없어 건너뛴다.
