...
> **표본 범위 — 공개 채널 전용.** 2023~2026 전 구간에서 **공개 채널 메시지만** 시기별(반기 단위)로 뽑아냈다. DM, private 채널(개인 대화·팀 내부 사담)은 프라이버시 보호를 위해 **의도적으로 제외**했다. 이 모델이 재현하는 건 작성자의 "공개 업무 채널 말투"다 — 팀 채널, 프로젝트 채널, 워킹그룹, 공지 채널 등. **핵심 관찰: 어투 시그니처는 시간 무관하게 일관된다** (애교 종결·물결·말줄임표·반문형이 2023~2026 동일).
