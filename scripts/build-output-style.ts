 ...
 *
 * 넣을 룰을 고르는 기준: 탐지기가 없어 사후 검사로 못 잡는 룰 중에 리뷰 코멘트나
 * 대화에서 실제로 반복해 걸린 것. 이 목록은 항상 켜진 프롬프트라 한 줄이 매 세션 비용이다.
 * 룰북에만 두고 게이트에 안 넣는 것도 정상 상태다 — 룰을 더할 때 자동으로 따라오지 않는다.
 */
