...
- **`F-9`가 원인 관계를 비유로 뭉갠 자리도 봐요.** 리뷰 답글에 "남은 r: 2건은 뿌리가 하나예요"라고 쓴 게 시작이었어요. 원인이 같다는 말을 뿌리에 얹으니 무엇이 같은지가 안 읽혀요.
  - 처음엔 영어 관용 비유의 직역을 잡는 `F-11`을 따로 세웠다가 두 라운드 리뷰를 거쳐 접었어요. 축이 어원이면 판정이 어원 판정이 되는데, "뿌리"의 근본 뜻은 표준국어대사전에 비유 뜻풀이로 실려 있어서 root cause를 옮긴 말인지 한국어 자체의 말인지를 쓰는 사람이 그 자리에서 확인할 수 없어요
  ...
  - **자료구조나 명세가 그 이름으로 부르는 자리는 대상이 아니에요.** 이 레포는 트리와 스레드의 root를 "뿌리"라 부르고 그런 자리가 17곳 있어요(src 8곳, tests 8곳, `plugin/skills/ship/SKILL.md` 1곳). 구조를 부르는 이름이라 가리킬 데가 있어요
...
- **`B-3`에 직역 처방을 한 줄 붙였어요.** "로우 행잉 프룻"은 "낮은 가지 열매"가 아니라 "금방 되는 일"이고 "실버 불릿"은 "은탄환"이 아니라 "만능 해법"이에요. 음차를 고치라고만 적어두면 직역으로 도망갈 자리가 남아요.
