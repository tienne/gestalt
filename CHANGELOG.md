...
- **`F-9`의 "뿌리" 예외를 root가 그 대상 이름인 자리로 좁혔어요.** 전 경계는 "자료구조나 명세가 그 이름으로 부르는 자리"였는데 읽는 쪽이 구조를 부르는 이름인지 판단해야 해서 흐렸습니다. 그 틈에 root와 무관한 "뿌리"가 예외로 남아 있었어요.
  ...
  - 룰 상세는 `F-9` 오적용 주의 문단 한 자리에만 뒀습니다.
- **`F-10`의 형태 예외에 피동 "박히다"를 넣었어요.** 그 행이 "형태로 빠지는 예외는 앞에 못이 없는 박아두다와 명사 잠금뿐이다"라고 적어뒀는데, 탐지기 정규식이 `못` 접두를 요구해서 "명세에 뜻이 박힌 말"도 실제로는 안 걸립니다. 선언이 탐지기보다 좁았어요.
  - 예외로 올린 근거는 **품사**입니다. 피동이라 못을 박는 주체가 문장에 안 서서, 이 룰이 세운 "누가 정하느냐" 축을 물을 대상이 없어요. 처음엔 "뜻을 정한 주체가 명세라 사람 쪽"이라고 적었다가 리뷰에서 걷혔습니다 — 그 근거대로면 명세는 문서라 같은 행의 대표 대상 예시까지 예외로 빠져나가요.
  - 판정 범위는 안 넓어졌어요. `박히다`는 이 선언 전에도 이미 안 걸렸고 레포의 "박" 어간 자리에 `못 박` 꼴이 새로 생기지도 않았습니다. 선언을 탐지기 실제 동작에 맞춘 거예요.
  - `rule-check-scope` 테스트에 두 케이스를 넣었어요. 탐지기에 박히다류 분기가 없다는 것과, 실물 잠금은 형태로 안 빠지고 걸린 뒤 사람이 가른다는 것입니다. 뒤쪽은 "사람 사이 합의" 사례와 대칭인데 그동안 비어 있었어요.
  - `src/humanize/detectors.ts` 주석도 형태 예외를 하나로 닫아 뒀던 걸 함께 고쳤습니다.
 표 행에 같은 내용을 다시 적었다가 두 사본이 갈리기 시작해서 걷었어요 — 이 문서가 스스로 "사본은 그 한 자리로 정한다"를 규범으로 적고 있습니다.
