...
- **output style에 F-9의 root 기준이 안 실리던 걸 고쳤어요.** `build-output-style`이 처방 칸을 앞 문장부터 담다가 28자를 넘으면 멈춥니다. 룰 68개 중 66개는 처방이 한두 문장이라 이걸로 충분한데 F-9(800자, 13문장)와 F-10(881자, 13문장)만 다른 모양이에요. 중앙값이 34자니까 이 둘이 23배입니다. root 기준은 F-9 처방의 아홉 번째 문장이라 상한을 올려도 사이 문장들이 먼저 자리를 채워서 영영 안 닿았어요.
  ...
  - **중복은 문자열이 아니라 문장 자리로 거릅니다.** 앞 요약은 영어 예시 축약과 `unchain`, `clamp`를 거쳐 원문과 달라져서 포함 검사로는 같은 문장인지 못 가릅니다. 표지 방식으로 짰을 때 이 검사가 조건부로 빗나가 같은 문장을 두 번 싣는 자리가 있었어요.
  ...
  - 올린 룰의 줄은 `HOIST_MAX`만큼 길어지고 그만큼이 매 세션 비용입니다. 안 올린 66개 룰의 줄은 한 글자도 안 바뀌어요.
