...
- **`F-9`의 "뿌리" 예외를 root가 그 대상 이름인 자리로 좁혔어요.** 전 경계는 "자료구조나 명세가 그 이름으로 부르는 자리"였는데 읽는 쪽이 구조를 부르는 이름인지 판단해야 해서 흐렸습니다. 그 틈에 root와 무관한 "뿌리"가 예외로 남아 있었어요.
  ...
  - **[0.76.1] 이전 기록이 explainer 두 자리를 "출처를 가리키는 자리라 대상 절 밖"이라고 적어뒀는데 그게 이번에 바뀌었어요.** `explainer/AGENT.md`의 "어투의 뿌리"와 `audience.md` 표 헤더의 "뿌리"는 값이 어느 문서에서 왔는지 적는 자리라 root가 아닙니다. 둘 다 "출처"로 고쳤어요. 예외로 인정했던 게 아니라 안 본 자리였습니다.
  ...
  - **탐지기는 이번에도 안 붙였어요.** "뿌리"만 놓고 보면 말 하나라 `F-10`의 "못 박" 그물처럼 형태로 걸 수는 있습니다. 그런데 레포에 root를 가리키는 정상 용례가 서른 자리 넘게 있어서 걸린 자리를 사람이 다시 보는 양이 검사로 얻는 것보다 커요. 별칭과 원인 쪽은 원래대로 뜻 판정이고요.
