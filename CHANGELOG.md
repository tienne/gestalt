...
- **따라야 할 기준이 레포 밖에 있어도 스킬이 읽습니다.** 게슈탈트는 규칙을 자기가 소유하는 전제로 만들어졌어요. 어투는 `ai-tell-quick-rules.md`, 주석은 `comment-rules.md`가 원본이고 `verify:rules`가 사본과 갈라졌는지 봅니다. 그런데 디자인 토큰은 디자인 시스템 서버가 갖고 있습니다. 조직 코딩 원칙은 다른 레포에 있고요. 베껴 오면 기준이 두 벌이 됩니다. 안 읽으면 아무 기준 없이 만든 결과가 기준을 지킨 결과와 똑같이 생겨서 리뷰가 잡을 때까지 아무도 모르고요.
  ...
  - 디자인 판정기(`src/design/`)와 `design-check` CLI는 이번에 안 들어갔습니다. `backup/design-check-escalated` 브랜치에 있어요. 게이트에 어떻게 물릴지를 먼저 정해야 합니다.
