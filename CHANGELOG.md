...
- **로컬 PR** — 원격 GitHub PR은 사람에게 넘기는 용도예요. 에이전트끼리 주고받는 데는 `gh`도 인증도 원격 왕복도 다 군더더기라 레포 안에서 끝내요.
  ...
  `gestalt pr` 13개 명령과 `ges_pr` 12개 액션이 생겼어요. 이벤트 소싱이라 상태를 따로 저장하지 않아요. 저장소는 `--git-common-dir` 기준으로 잡아서 워크트리 여럿이 `.gestalt/reviews.db` 하나를 공유해요. PR의 커밋은 `refs/gestalt/pr/<id>/head`가 붙잡아요 — 워커가 브랜치를 지워도 diff가 살아요.
