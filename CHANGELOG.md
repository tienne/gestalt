...
- **blast-radius에 git 이력 co-change 신호를 나란히 실었어요.** 코드 그래프가 만드는 엣지는 `CONTAINS`와 `IMPORTS_FROM`, `INHERITS` 셋뿐입니다. `EdgeKind.CALLS`와 `TESTED_BY`는 선언만 있고 여덟 언어 플러그인 어디서도 안 만들어요. 이름은 코드 그래프인데 실질은 import 그래프였습니다. 1-hop 정확도가 `grep -rl`과 사실상 같았고 depth를 5로 올리면 영향 파일이 전체 절반을 넘어서, 읽을 파일을 좁힌다는 목적에 정면으로 역행했어요.
  ...
  - **임계 둘을 SQL `WHERE`로 내렸어요.** 중간에 개수로 미리 줄이는 후보창을 뒀다가 걷어냈습니다. 정렬 키(`pair_count`)와 랭킹 키(`confidence × lift`)가 달라서 카운트는 낮고 점수는 높은 페어가 창 밖에서 통째로 유실됐어요. 그러면 보인 목록이 전체 순위의 상위 접두사라는 말이 성립하지 않습니다. 임계는 어차피 결과에서 뺄 행을 애초에 안 뜨는 것뿐이라 손실이 원리상 없고, 덕분에 `totalMatched`가 하한이 아니라 정확한 수가 됐어요.
  - **대신 `MAX_MATCHED_ROWS` 절대 천장을 SQL에 뒀습니다.** 임계 둘은 외부 입력이라 둘 다 0이면 전량 스캔이 되고, `limit`은 랭킹 뒤에 걸려서 읽는 행도 `stat` 팬아웃도 못 막아요. 천장은 10,000행이고 `exists` 확인과 점수 계산보다 앞, SQL 안에 있습니다. 값의 근거는 셋이에요 — 이 레포 최악이 4,597행이라 두 배 넘는 여유가 있고, 출력 상한 최대(500)의 스무 배라 천장이 사용자가 보는 목록을 정하는 자리가 안 되며, 걸려도 `existsSync` 호출 수가 유한합니다.
...
- **`ges_code_graph`에 `co_change` 액션.** `target`을 주면 그 파일과 함께 바뀐 이웃을, 생략하면 레포 전체 상위 페어를 돌려줘요. 기존 `blast_radius`와 `diff_radius`, `db_exists`가 전부 snake_case라 이름을 거기 맞췄습니다. 공개 면이라 릴리즈 뒤에는 되돌리기 어려운 자리예요.
  ...
  - **후속 티켓 대상이 하나 드러났어요.** `query` 액션의 `callers_of`와 `callees_of`가 런타임에서 늘 빈 결과를 냅니다. `CALLS` 엣지를 만드는 플러그인이 하나도 없고, `engine.ts`가 `function:${repoRoot}:${target}`로 노드를 찾는데 플러그인이 저장한 id는 `function:${filePath}:${name}`이라 서로 안 맞아요. 문서 예시를 소스와 대조하다 나온 자리라 이번 범위에 안 넣었습니다. 고치려면 플러그인 여덟 개에 호출 관계 추출을 더해야 해요.
