...
- **`F-9`가 원인 관계를 비유로 뭉갠 자리도 봐요.** 리뷰 답글에 "남은 r: 2건은 뿌리가 하나예요"라고 쓴 게 시작이었어요. 원인이 같다는 말을 뿌리에 얹으니 무엇이 같은지가 안 읽혀요.
  ...
  - `SPOTLIGHT`와 게이트는 안 건드렸어요. `F-9`가 이미 양쪽에 있고 새 룰을 안 세웠으니 항상 켜진 프롬프트도 그대로예요
- **`B-3`이 직역형도 대상으로 봐요.** "로우 행잉 프룻"은 "낮은 가지 열매"가 아니라 "금방 되는 일"이에요. "실버 불릿"도 같은 자리에서 "만능 해법"을 가리켜요. 음차만 고치라고 적어두면 직역으로 도망갈 자리가 남아서 패턴 칸에 직역형을 함께 올렸어요. 화이트리스트는 낱말 단위라 "불릿"이 정착어여도 "실버 불릿"은 이 룰이 봐요.
