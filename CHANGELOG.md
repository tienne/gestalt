...
- **결함을 은유로 부르는 자리에 `F-9`를 세웠어요.** 리뷰에 "맨몸 `setBoundVariableForPaint`"라고 쓴 별칭을 PR 작성자가 답글에서 그대로 받아썼어요. 코멘트 한 줄 고치는 것보다 되돌리기 어려운 자리예요.
  ...
  - **예외는 남이 정의해 둔 이름을 가져다 쓸 때뿐이에요.** "널 포인터"나 "데드락"처럼 명세와 교과서에 뜻이 박혀 가리킬 데가 있는 말이에요. 팀에서 몇 번 쓰였다는 건 예외가 아니에요 — 그렇게 열면 답글에 한 번 재생산된 별칭이 곧바로 빠져나가는데 그게 이 룰이 막으려는 자리예요
  - 탐지기는 안 붙였어요. `C-14`와 `E-8`, `G-4`와 같은 이유로 뜻을 봐야 갈려요. 표 안에 그 사실을 적어뒀어요
