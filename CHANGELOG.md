...
- **`review-loop` 스킬.** 남의 GitHub PR을 리뷰어로 따라가는 루프예요. 리뷰하고 인라인 코멘트를 남기고 판정을 게시한 뒤, 작성자가 대응했는지 보고 재리뷰합니다. `ship`이 내 PR을 통과시키는 루프라면 이건 그 거울상이에요.
  ...
  - 판정에 쓰는 수는 전부 "상태 조회" 한 곳에서 나와요. 스레드를 전량 받고(커서 루프, 페이지 상한 50장) 그 스냅샷으로 셉니다. 조회가 실패하면 수를 안 만들고 멈춰요 — `jq -s`는 파일이 없어도 `0`을 찍고 종료 코드 2로 끝나서, 그냥 두면 조회 실패가 "스레드 0개"로 읽혀 승인이 나갑니다.
