...
- **output style에 F-9의 root 기준이 안 실리던 걸 고쳤어요.** `build-output-style`이 처방 칸을 앞 문장부터 담다가 28자를 넘으면 멈춥니다. 처방이 두 문장 이하인 룰이 46개고 세 문장 이상이 22개인데, F-9(800자, 13문장)와 F-10(881자, 13문장)은 그다음으로 긴 B-3(585자)의 1.4배고 문장 수로는 13 대 8이에요. 중앙값이 34자니까 이 둘이 23배입니다. root 기준은 F-9 처방의 아홉 번째 문장이라 상한을 올려도 사이 문장들이 먼저 자리를 채워서 영영 안 닿았어요.
  ...
  - **`tsconfig`의 `include`에 `scripts`를 넣었어요.** 이번 변경에서 `Rule` 타입 import를 빠뜨렸는데 `pnpm gate`가 초록이었습니다. `include`가 `src`랑 `bin`뿐이라 `scripts/`는 타입 검사 대상이 아니에요. eslint도 타입을 안 보고 tsx는 타입을 지우고 돌립니다. 넣으니 `scripts/simulate-lateral.ts`의 옛 필드(`ambiguityScore`)가 함께 드러나서 `resolutionScore`로 고쳤어요 — 스키마가 `1 - ambiguityScore`로 환산하니 0.15는 0.85입니다. 빌드 범위는 `tsconfig.build.json`으로 갈랐습니다. `build`는 `--noEmit`이 아니라서 안 가르면 `dist/scripts/` 28개가 npm 패키지에 실려 나가요. `tests/`는 기존 타입 오류가 55개라 이번엔 그대로 뒀습니다.
