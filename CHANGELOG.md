...
- **`gestalt humanize-check`** — 윤문 전후 두 파일을 넣으면 변경률, S1 잔존, 보호 토큰 생존, 구조 보존을 각각 재고 exit code로 답한다.
  ...
  모델이 스스로 매긴 변경률과 등급은 참고값이라 과윤문을 실제로 막지 못했다. 문자 기반 변경률 하나로는 구조 편집도 안 보인다. 변경률 2.8%인데 문장 3할이 갈려나간 경우가 있었다.
...
- **어투 룰 심각도 재조정** — AI 산문 60편과 2022년 이전 한국어 산문 60편을 대조한 결과를 반영했다. 원어민이 오히려 더 쓰는 표현을 무조건 지우면 사람 글이 AI 글이 된다.
  ...
  앞의 두 룰은 대화와 리뷰 코멘트에서 S1로 남겼다. 잰 게 문서 산문이라 말투에는 그대로 안 붙는다.
...
- **화이트리스트 밖 음차 정리** — 대표 예시가 "소스 오브 트루스"인데 정작 문서들이 약어로 우회하고 있었다.
  ...
  "레이어 → 계층"을 일괄 치환했더니 받침이 생기면서 조사가 두 군데 깨졌다 (`계층라면`, `계층(…)는`). 찾아 바꾸기로 밀 수 있는 작업이 아니다.
...
- **이슈별 코드 스니펫** — 리뷰 리포트가 각 이슈 아래에 해당 라인 주변 코드를 붙인다. 지목한 라인에는 `>` 마커가 붙는다.
  ...
  고정 3줄은 코드마다 안 맞아서 위아래 5줄에서 시작해 들여쓰기로 감싸는 블록을 추정해 창을 조정한다.

  - 위로는 감싸는 선언을 2단계까지 붙인다. 함수 안 for 문에서 걸린 이슈면 for와 함수 선언이 함께 보인다. 이슈가 어느 함수 안에 있는지가 제일 알고 싶은 정보인데 고정 창으로는 선언이 잘려 나갔다
...
- **AI-tell 탐지 모드 (humanize-monolith)** — 원문을 한 글자도 바꾸지 않고 룰 ID와 원문 인용, 한 줄 처방만 돌려준다. "이거 AI 같아?", "패턴만 짚어줘" 같은 요청이 이쪽으로 온다.
  ...
  - 교정문을 예시로도 붙이지 않는다. 붙이면 사용자가 복붙해서 사실상 윤문 모드가 된다
  - AI가 썼는지 판정하지 않는다. 이름 붙은 패턴은 사용자가 확인할 수 있는 근거지만 저자 추측은 근거가 없다
  - A~D 등급을 매기지 않는다. 등급은 윤문 결과 품질 지표라서 남의 원문에 붙이면 점수질이 된다. S1과 S2 개수만 센다
...
- **삭제 처방은 삭제다** — 처방에 "삭제"가 적힌 항목에서 모델이 리듬을 살리려고 새 마무리 문장이나 새 부제를 지어 넣는 일이 잦았다. 원문에 없던 수사를 더하는 것이라 자체검증에 걸리는데도 룰 문구가 "삭제 또는 구체화"라 재작성 여지를 남겨뒀다.
  ...
  - `본질적으로` 같은 부사는 다른 부사로 갈아끼우지 말고 그냥 뺀다
...
- **Codex 슬래시 커맨드 6개** — `skills`만 내보내면 Codex의 `/` 목록에 안 떠서 자연어나 `$gestalt:review` 멘션으로만 부를 수 있었다. `commands/` 디렉토리를 두면 파일명이 그대로 슬래시 커맨드가 되고 매니페스트에 따로 선언할 필요는 없다.
...
- **"실측"을 걷어냈다** — code-review-writer가 이 문서를 필수로 읽다 보니 실제 PR 리뷰에 "레포 실측으로 1444개 중"으로 두 번 새어나갔다. 세어본 것을 그렇게 부르면 뜻은 맞고 단어만 튀지만, 대조한 것을 그렇게 부르면 뜻 자체가 어긋난다. 후자는 근거 과장이라 리뷰이가 검증 없이 받아들이는 쪽으로 간다.

## [0.46.0] - 2026-08-04

### Added

- **리뷰 코멘트 헤지 밀도 상한** — "~것 같아요"를 보존 대상으로만 규정해두니 모든 문장에 붙어서, 문장 하나하나는 자연스러운데 리뷰 전체가 기계로 읽혔다. 코멘트당 1회, 리뷰 전체의 절반 이하로 상한을 뒀다. 분포는 문장 단위로 안 잡혀서 자가점검 8개 항목을 따로 뒀다.
- **원문 음차 처리 규칙** — 안 굳은 음차(에스케이프 해치, 스파이크)가 리뷰 대상 원문에 있다는 이유로 화이트리스트를 통과하던 것을 막았다. 코드블록과 인용은 그대로 두고 리뷰어 자기 문장은 첫 등장에 풀어쓴다.
...
- **SQLite 연결에 `busy_timeout` 설정** — `journal_mode`와 `foreign_keys`만 걸고 `busy_timeout`을 안 걸어서 프로세스가 둘 이상 같은 DB에 쓰면 잠금이 풀리기를 기다리지 않고 SQLITE_BUSY로 즉시 실패했다. pragma는 연결마다 설정해야 적용된다.
