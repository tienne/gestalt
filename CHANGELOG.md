...
- **`F-9`의 "뿌리" 예외를 root가 그 대상 이름인 자리로 좁혔어요.** 전 경계는 "자료구조나 명세가 그 이름으로 부르는 자리"였는데 읽는 쪽이 구조를 부르는 이름인지 판단해야 해서 흐렸습니다. 그 틈에 root와 무관한 "뿌리"가 예외로 남아 있었어요.
  ...
  - **이 기준이 F-9 일반 예외보다 앞섭니다.** `root cause analysis`는 SRE 문헌에 뜻이 박힌 말이라 "명세나 교과서에 뜻이 박힌 말은 통과"라는 일반 예외로는 통과 자격이 있어요. 같은 행 안에서 두 예외가 같은 대상에 반대 판정을 내던 자리라 우선순위를 박았습니다.
  ...
  - **탐지기는 이번에도 안 붙였어요.** "뿌리"만 놓고 보면 말 하나라 `F-10`의 "못 박" 그물처럼 형태로 걸 수는 있습니다. 그런데 정상 용례가 너무 많아서 걸린 자리를 사람이 다시 보는 양이 검사로 얻는 것보다 커요. 게슈탈트 레포 기준 `src`와 `tests`와 `plugin/skills`의 "뿌리"가 35곳입니다(룰 문서의 인용은 뺀 수). [0.75.0]이 적은 17곳보다 는 건 그때는 트리와 스레드의 root만 셌고 이번엔 디렉토리와 프로젝트와 git ref의 root까지 예외로 넣었기 때문이에요. 별칭과 원인 쪽은 원래대로 뜻 판정이고요.
