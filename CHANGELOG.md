...
- **절 참조 표기가 규칙이 되고 검사에 걸려요.** 마크다운끼리는 앵커 링크(`[실측 근거](ai-tell-quick-rules.md#실측-근거)`), 소스 주석은 `파일.md §「제목 전문」`으로 씁니다. `pnpm verify:rules` 8번이 앵커와 제목이 실제로 있는지 봐요. 그동안 `§`는 아무도 검사하지 않아서 제목을 고쳐도 참조만 낡았어요 — 기존 참조 열 곳 중 일곱 곳이 이미 어긋나 있었어요. README의 죽은 앵커도 이 검사가 처음 잡았고요. 규칙은 `style-guide.md`의 "다른 문서의 절을 가리킬 때"에 적어뒀어요.
