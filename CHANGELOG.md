...
- **주석 위생 검사 (quality-reviewer)** — `/review`가 코드 품질을 볼 때 주석도 함께 본다. 코드를 읽거나 `git log`로 확인되는 내용은 주석으로 남길 이유가 없다. 코드가 바뀔 때 같이 안 고쳐져 결국 사실과 어긋난다.
  ...
  앞의 넷은 읽는 시간만 쓰지만 뒤의 둘은 읽는 사람을 잘못된 방향으로 끌어서 심각도를 높였다.

  남기는 주석은 왜 이렇게 짰는지 하나뿐이다. 외부 API 버그 우회, 성능 제약, 겉보기에 틀려 보이는 코드가 의도된 것이라는 근거, 그리고 공개 API와 공용 유틸의 JSDoc.

  코멘트는 지우자는 말로 끝나지 않고 대체안까지 낸다. 이름 풀어쓰기, 블록을 함수로 빼기, 매직 넘버를 이름 붙은 상수로 올리기, 배경이 길면 README나 ADR로 옮기고 링크만 남기기.

  PR에 인라인으로 게시할 때는 강제성 접두어가 붙는다. `high`는 `r:`(꼭 반영), `warning`은 `c:`(웬만하면 반영).

## [0.52.0] - 2026-08-06

윤문 결과를 모델 자평이 아니라 코드로 검증한다.

> 이 릴리즈부터 어투 룰 용어가 자주 나온다. **S1**은 반드시 고쳐야 하는 룰, **S2**는 문맥을 보고 판단하는 룰이다. **음차**는 영어를 소리대로 옮긴 말(에스케이프 해치 등), **윤문**은 뜻을 유지한 채 어투만 다듬는 작업이다.

### Added

- **`gestalt humanize-check`** — 윤문 전후 두 파일을 넣으면 변경률, S1 잔존, 보호 토큰 생존, 구조 보존을 각각 재고 exit code로 답한다.

  ```bash
  gestalt humanize-check --before draft.md --after revised.md --register chat
  ```

  모델이 스스로 매긴 변경률과 등급은 참고값이라 과윤문을 실제로 막지 못했다. 문자 기반 변경률 하나로는 구조 편집이 드러나지 않는다. 변경률 2.8%인데 문장 3할이 통째로 바뀐 경우가 있었다.

  - 룰 목록은 `ai-tell-quick-rules.md`를 읽어서 만든다. 코드에 룰을 복사하지 않는다
  - 정규식으로 오탐 없이 셀 수 있는 21개만 탐지한다. 뜻을 봐야 하는 룰은 뺐다
  - `--register`가 `doc`이냐 `chat`이냐에 따라 S1 대상이 달라진다
  - 변경률은 어절 단위 LCS로 잰다. 6천 어절이 넘으면 겹침 비율로 떨어진다
  - 보호 토큰(수치, 인용, 코드, URL)이 유실되면 경고가 아니라 채택 금지다

- **`pnpm verify:rules`** — 룰북과 에이전트 문서 14곳의 룰 ID와 심각도가 어긋났는지 본다. 기준 문서에서 ID를 지우거나 심각도를 바꿔도 손으로 옮겨 적은 사본은 그대로 남아 아무도 모르는 채 어긋난다. 처음 돌렸을 때 7개가 어긋나 있었다.

  룰북에 없는 ID 인용, 자체검증 목록에서 빠진 S1, 표와 목록의 심각도 불일치, S1 자리에 섞인 S2, 룰 문서 본문의 금지 표현을 잡는다. `pnpm test`와 `pnpm build` 양쪽에 걸려 있다.

### Changed

- **어투 룰 심각도 재조정** — AI 산문 60편과 2022년 이전 한국어 산문 60편을 대조한 결과를 반영했다. 원어민이 오히려 더 쓰는 표현을 무조건 지우면 사람 글이 AI 글이 된다.
  ...
  앞의 두 룰은 대화와 리뷰 코멘트에서 S1로 남겼다. 측정 대상이 문서 산문이라 말투에는 그대로 적용되지 않는다.

- **굳은 음차를 화이트리스트에 넣었다** — 소스, 롤백, 파싱, 레지스트리, 불릿. 업계에서 굳은 말인데 목록에 없어 이론상 위반으로 잡혔다. 즉석 조합인 "불릿 리스트"만 "불릿 목록"으로 바꿨다. 화이트리스트나 체크리스트처럼 통째로 하나가 된 합성어는 그대로 뒀다.

- **화이트리스트 밖 음차 정리** — 대표 예시가 "소스 오브 트루스"인데 정작 문서들이 약어로 우회하고 있었다.
  ...
  "레이어 → 계층"을 일괄 치환했더니 받침이 생기면서 조사가 두 군데 깨졌다 (`계층라면`, `계층(…)는`). 찾아 바꾸기 한 번으로 끝낼 수 있는 작업이 아니다.

- **지라 티켓 템플릿 헤딩** — `## 인수조건 (AC)` → `## 완료 조건`. WDS 전체에 "인수조건"이 5개뿐이었고 그중 넷이 한 묶음이라 팀이 쓰는 말이 아니었다. 본문의 약칭 AC도 함께 걷어 용어를 하나로 뒀다.

## [0.51.0] - 2026-08-06

에이전트 tier가 라벨을 넘어 실제 모델 선택으로 이어진다.

### Changed

- **tier → 모델 해석** — 에이전트 frontmatter의 `tier`가 그동안 라벨로만 남아 있었다. tier를 모델로 바꾸는 라우터는 있었지만 인스턴스화하는 곳이 0곳이라, `frontier`로 선언한 architect, harness-architect, continuity-judge가 아무 효과가 없었다.

  이제 `ges_agent get`이 tier와 함께 해석된 `model`을 돌려준다. 스킬이 서브에이전트를 띄울 때 그 값을 Agent 도구 `model` 파라미터로 넘긴다. 세션에서 직접 수행하면 tier는 여전히 참고값이다.
  ...
  `tierModels` 대신 환경변수 `GESTALT_TIER_MODEL_*`로도 덮어쓴다. 이 값은 API 모델 ID가 아니라 호스트 Agent 도구가 받는 별칭이라 `llm.model`과 층위가 다르다.

- **기본 모델 `claude-sonnet-4-6` → `claude-sonnet-5`** — 옛 모델을 그대로 쓰려면 `gestalt.json`의 `llm.model`에 명시한다. 문서에 남아 있던 deprecated 모델 ID 4곳도 함께 정리했다.

### Removed

- **죽은 모델 라우팅 경로** — `FiguralRouter`는 인스턴스화하는 곳이 0곳이었고 `resolvePromptModel`은 결과를 읽어갈 훅이 레포에 없었다. 둘 다 `index.ts`에서 내보내지 않아 패키지 공개 API는 그대로다.

## [0.50.0] - 2026-08-06

### Changed

- **공유 룰북 위치 이동** — author-voice, ai-tell-quick-rules, style-guide 세 문서를 `plugin/role-agents/_shared/references/`로 옮겼다. 다섯 개가 넘는 에이전트가 함께 쓰는데 경로가 technical-writer 하위였다. author-voice는 헤더에 "공유 레퍼런스"라고 적어두고도 경로가 그걸 반영하지 않았다.

  `plugin/skills/_shared`와 같은 규칙이다. `AGENT.md`가 없어 레지스트리가 에이전트로 로드하지 않는다. 참조 26곳을 고치면서 원래 깨져 있던 링크 2개도 같이 고쳤다.

## [0.49.1] - 2026-08-06

### Fixed

- **"수준"으로 정도를 뭉개지 않는다** — "매끄럽게 다듬는 수준이고"는 등급 어감이 실려 채점처럼 읽힌다. 범위를 말할 때는 "정도"로 쓰고 가능하면 형식명사를 빼고 동사로 푼다.
- **세는 단위 룰(I-5) 오작동 가드** — "안 돌려본 건 PR에 적어두셔서"의 "건"은 "것은"의 준말이라 늘려 쓰면 오히려 딱딱해지는데, 글자로 매칭하면 이걸 잘못 고쳤다. 세는 단위일 때만 고치도록 두 층위를 표로 나란히 뒀다.
- **가운뎃점을 금지하는 문서가 본문 산문에서 가운뎃점을 쓰고 있었다** — 기존 문서의 용어 목록은 예외지만 산문 나열은 예외가 아니다.

## [0.49.0] - 2026-08-06

리뷰 리포트에서 이슈 라인을 코드와 함께 읽는다.

### Added

- **이슈별 코드 스니펫** — 리뷰 리포트가 각 이슈 아래에 해당 라인 주변 코드를 붙인다. 지목한 라인에는 `>` 마커가 붙는다.

  고정 3줄은 코드마다 맞지 않아서 위아래 5줄에서 시작해 들여쓰기로 감싸는 블록을 추정해 창을 조정한다.

  - 위로는 감싸는 선언을 2단계까지 붙인다. 함수 안 for 문에서 걸린 이슈면 for와 함수 선언이 함께 보인다. 이슈가 어느 함수 안에 있는지가 가장 알고 싶은 정보인데 고정 창으로는 선언이 잘려 나갔다
  - 아래로는 감싸는 블록이 끝나는 라인에서 멈춘다. 다음 함수를 넘보지 않는다
  - 창과 선언 사이가 잘리면 생략 표시(…)를 넣는다

### Fixed

- **리뷰 코멘트를 "지적"이라 부르지 않는다** — 상대를 잡아세우는 뉘앙스가 붙어 제안형 어투와 어긋난다. 내가 남긴 것은 "남겼던 의견", 상대가 남긴 것은 "짚어주신 부분"으로 쓴다. 룰 문서가 스스로 쓰던 "지적"도 함께 걷어냈다. 금지 어휘를 본문에서 쓰면 산출물로 새기 때문이다.
- **식별자 뒤 조사 붙여쓰기** — 리뷰 코멘트는 커밋 해시나 브랜치명을 문장에 그대로 섞어서 조사 처리가 반복적으로 어긋났다. `6564d04 에서` → `6564d04에서`, `c: 로` → `c:로`. 어투가 아니라 맞춤법이라 윤문 등급과 무관하게 고친다.
- **한자어 명사와 추상명사 이동 동사** — "강등 방향도 맞게 갔습니다" → "심각도 내린 것도 맞습니다". 방향이나 결론 자체가 가는 게 아니라 무언가가 그 방향으로 가는 것이라, 사람이나 대상을 주어로 되돌린다.

### Documentation

- **릴리즈 절차에 태그 이동 단계(5.5) 추가** — `npm version`이 찍는 태그는 매니페스트 커밋보다 앞을 가리키는데, Actions는 태그를 체크아웃해 빌드한다. 그대로 두면 배포 패키지에 한 버전 뒤처진 매니페스트가 실린다. v0.47.1 태그에 0.47.0 매니페스트가 실려 나간 적이 있다. 푸시 전에만 안전하다는 조건도 함께 적었다.

## [0.48.0] - 2026-08-06

윤문 말고 짚어만 달라는 요청을 받는다.

### Added

- **AI-tell 탐지 모드 (humanize-monolith)** — 원문을 한 글자도 바꾸지 않고 룰 ID와 원문 인용, 한 줄 처방만 돌려준다. "이거 AI 같아?", "패턴만 짚어줘" 같은 요청이 이쪽으로 온다.

  탐지 모드에서 막아둔 것:

  - 교정문을 예시로도 붙이지 않는다. 붙이면 사용자가 그대로 복사해 쓰게 되어 사실상 윤문 모드가 된다
  - AI가 썼는지 판정하지 않는다. 이름 붙은 패턴은 사용자가 확인할 수 있는 근거지만 저자 추측은 근거가 없다
  - A~D 등급을 매기지 않는다. 등급은 윤문 결과 품질 지표라서 남의 원문에 붙이면 점수를 매기는 셈이 된다. S1과 S2 개수만 센다

- **`pnpm verify:plugin`** — `dist/plugin`이 원본과 바이트 단위로 같은지 확인한다. npm 설치 환경의 런타임이 실제로 읽는 자산은 `plugin/`이 아니라 `dist/plugin/`인데, postbuild가 `cp -r`만 해서 삭제를 반영하지 못했다. `plugin/`에서 지운 스킬이 dist에 남아 배포되면 유령 스킬이 로드된다.

  postbuild 앞에 `rm -rf dist/plugin`을 넣어 원인을 막고 복사 후 한 번 더 확인한다. 복사 누락, dist에만 남은 파일, 내용 불일치를 각각 보고하고 하나라도 있으면 빌드를 끊는다.

### Changed

- **삭제 처방은 삭제다** — 처방에 "삭제"가 적힌 항목에서 모델이 리듬을 살리려고 새 마무리 문장이나 새 부제를 지어 넣는 일이 잦았다. 원문에 없던 수사를 더하는 것이라 자체검증에 걸리는데도 룰 문구가 "삭제 또는 구체화"라 재작성 여지를 남겨뒀다.

  - 콜론 부제는 콜론 뒤를 버리고 앞부분만 남긴다. 부제를 갈아끼우면 패턴이 그대로 남는다
  - hype 어휘는 원문에 수치가 없으면 만들지 말고 수식어만 지운다
  - `본질적으로` 같은 부사는 다른 부사로 갈아끼우지 말고 삭제한다

  자체검증 항목이 6개에서 7개로 늘어난 만큼 등급 기준도 맞췄다. A는 7항, B는 6항 이상, C는 5항 이하.

## [0.47.1] - 2026-08-04

### Added

- **Codex 슬래시 커맨드 6개** — `skills`만 내보내면 Codex의 `/` 목록에 뜨지 않아 자연어나 `$gestalt:review` 멘션으로만 부를 수 있었다. `commands/` 디렉토리를 두면 파일명이 그대로 슬래시 커맨드가 되고 매니페스트에 따로 선언할 필요는 없다.

  이름에 `gestalt-` 접두어를 붙였다. Codex에 내장 review 명령이 있어 `/review`로 두면 충돌한다. 각 커맨드는 인자와 요약만 담고 절차는 해당 `SKILL.md`를 읽게 위임해서, 스킬을 고칠 때 커맨드까지 같이 고쳐야 하는 상황을 만들지 않았다.
...
- **Codex 플러그인 매니페스트** — `codex plugin add`로 MCP 서버와 워크플로 스킬 18개를 한 번에 받는다. 그전까지 Codex 사용자는 MCP 도구만 쓸 수 있어서 review 같은 스킬을 쓰지 못했다. 설치 크기는 680KB다.

  - `.agents/plugins/marketplace.json` — Codex는 이 경로에서만 마켓플레이스를 찾는다. `.codex-plugin/marketplace.json`은 인식하지 않는다
  - `plugin/.codex-plugin/plugin.json` — `skills`와 `mcpServers` 키는 Claude Code 형식과 같고 `interface` 블록만 Codex 전용이다
  - `plugin/mcp.json` — npx로 받아 실행하므로 설치 위치에 의존하지 않는다

### Changed

- **배포 자산을 `plugin/` 하나로 모았다** — Codex는 마켓플레이스가 가리킨 디렉토리를 통째로 복사한다. 레포 루트를 가리키면 `.git`과 `node_modules`까지 딸려가 1.6GB가 되고 심링크는 따라가지 않아 자산을 링크로 공유할 수도 없었다.

  두 클라이언트가 같은 디렉토리를 각자 매니페스트로 가리키므로 복사본이 생기지 않는다. skills와 role-agents를 함께 옮겨서 review 스킬의 `../../role-agents/` 참조는 상대 깊이가 그대로다. `src/core/config.ts`의 자산 디렉토리 기본값 다섯 개가 함께 바뀌었다.

### Fixed

- **버전 동기화에서 빠진 Codex 매니페스트** — `npm version` 후 `plugin/.codex-plugin/plugin.json`만 옛 버전으로 남았다. `codex plugin list`가 이 파일의 `version`을 보여줘서 설치된 플러그인 버전이 실제와 달라 보였다.

## [0.46.1] - 2026-08-04

### Fixed

- **어투 문서가 자기가 금지한 말을 쓰고 있었다** — 에이전트가 그걸 배워 산출물로 내보내는 경로다. F-7이 "증류"를 금지하는데 문서 7곳이 그 단어를 쓰고 있던 게 대표적이다.
  ...
  금지어 예시로 인용하는 자리는 그대로 뒀다.

- **"실측"을 걷어냈다** — code-review-writer가 이 문서를 필수로 읽다 보니 실제 PR 리뷰에 "레포 실측으로 1444개 중"으로 두 번 새어나갔다. 세어본 것을 그렇게 부르면 뜻은 맞고 단어만 어색하지만 대조한 것을 그렇게 부르면 뜻 자체가 어긋난다. 후자는 근거 과장이라 리뷰이가 검증 없이 받아들이는 쪽으로 간다.

## [0.46.0] - 2026-08-04

### Added

- **리뷰 코멘트 헤지 밀도 상한** — "~것 같아요"를 보존 대상으로만 규정해두니 모든 문장에 붙어서 문장 하나하나는 자연스러운데 리뷰 전체가 기계로 읽혔다. 코멘트당 1회, 리뷰 전체의 절반 이하로 상한을 뒀다. 분포는 문장 단위로 잡히지 않아 자가점검 8개 항목을 따로 뒀다.
- **원문 음차 처리 규칙** — 굳지 않은 음차(에스케이프 해치, 스파이크)가 리뷰 대상 원문에 있다는 이유로 화이트리스트를 통과하던 것을 막았다. 코드블록과 인용은 그대로 두고 리뷰어 자기 문장은 첫 등장에 풀어쓴다.

## [0.45.0] - 2026-07-31

실행 태스크를 외부 런타임 터미널로 뿌린다.

### Added

- **`dispatch` 스킬** — execute의 실행 단계를 외부 에이전트 런타임 터미널로 돌리는 opt-in 백엔드다. 계획 수립과 평가, 개선은 그대로 execute가 담당한다.

  병렬 자체가 목적이 아니다. execute가 이미 Agent 도구로 병렬을 돌린다. 이 스킬로 얻는 건 워커별로 다른 에이전트 CLI, 사람이 들여다볼 수 있는 터미널, `worker_done` 생애주기 셋이고 필요 없으면 기본 경로가 더 가볍다.

  - 런타임 감지에 실패하면 흉내내지 않고 기본 경로를 권하며 멈춘다
  - 리눅스에서 `orca`는 GNOME 스크린리더 이름이라 존재 확인이 아니라 status 응답으로 판단한다
  - 워커는 캐시된 세션 상태를 믿지 않고 `worker_done`마다 다시 읽는다. 서버 프로세스마다 인메모리 세션을 따로 들고 있어서다

### Fixed

- **SQLite 연결에 `busy_timeout` 설정** — `journal_mode`와 `foreign_keys`만 걸고 `busy_timeout`을 걸지 않아 프로세스가 둘 이상 같은 DB에 쓰면 잠금이 풀리기를 기다리지 않고 SQLITE_BUSY로 즉시 실패했다. pragma는 연결마다 설정해야 적용된다.

  이벤트 DB는 홈 글로벌 경로(`~/.gestalt/events.db`)이고 MCP 서버는 클라이언트 세션마다 프로세스가 따로 뜬다. 같은 레포에서 창을 두 개만 열어도 이미 겹친다. 코드 그래프 DB도 같이 걸었다. post-commit 훅이 증분 빌드를 도는 동안 다른 창에서 blast radius를 돌리는 상황이 실제로 생긴다.

  WAL 설정 직후에 넣었다. 앞에 두면 `journal_mode` 전환 자체가 경합에 걸린다.
...
- **`review-reply` 스킬** — PR에 달린 리뷰 코멘트를 수집해 유형별로 처리하고 답글을 인라인으로 게시한다. review 스킬은 diff를 읽어 리뷰를 만드는 방향이라 게시 경로가 `pulls/{n}/reviews`뿐이었고 답글은 스레드 API(`comments/{id}/replies`)라 붙일 데가 없었다.

  - 미해결 스레드는 REST가 resolved 여부를 주지 않아 GraphQL `reviewThreads`로 조회한다
  - 승인 단계를 두 군데 뒀다. 무엇을 수용하고 무엇을 반박할지 정하는 유형 분류, 그리고 게시 직전 답글 미리보기
  - accept로 분류했는데 커밋이 없으면 답글을 쓰지 않는다. "반영했습니다"는 리뷰어가 approve 근거로 삼는 사실 주장이라서다
  - 스레드 resolve 기본값은 false다. 해결 판단은 리뷰어 몫이다

- **`code-review-responder` 에이전트** — 리뷰 받는 쪽 답글 전담. 기존 code-review-writer는 리뷰어 관점 전용이라 severity 판정과 `r:`/`c:`/`a:` 접두어가 답변에 맞지 않았다.
- **`sessionId` 셀렉터** — `active`와 `latest`로 세션을 지정한다.

### Documentation

- **`_shared/` 공유 규칙** — 외부에서 읽어온 텍스트를 다루는 규칙과 도구가 없을 때의 대응을 스킬마다 반복해 적지 않도록 뺐다.

## [0.43.0] - 2026-07-30

지금 동시에 붙일 수 있는 태스크를 호스트가 안다.

### Added

- **`nextTaskIds`** — 착수 가능한 태스크 집합을 `execute_task`, `resume`, `status` 세 경로에 싣는다. 2개 이상일 때만 안내 문구를 덧붙인다. 실행 자체는 그대로 순차이고 병렬로 붙일지는 호스트가 판단한다.

  기존 `nextTaskId`는 이 배열의 첫 원소에서 파생되므로 둘의 의미가 어긋나지 않는다. 세션 갱신과 이벤트 replay가 `computeReadyTaskIds` 한 함수를 쓴다. 순회 기준은 `topologicalOrder`다. 순환이나 자기참조가 있으면 validator가 이 배열을 비우므로 빈 집합이 나온다.

  replay 경로가 그동안 `nextTaskId`를 초기값 null에서 갱신하지 않아, 서버 재시작 후 `ges_status`는 null인데 `resume`은 정상값을 내보내 같은 세션에 두 응답이 어긋났다. 이번에 함께 풀렸다.

### Documentation

- **스킬 description에 경계 선언 추가** — 겹치는 스킬이 여럿인데 언제 쓸지만 적혀 있어 오발동 여지가 있었다. 어느 스킬을 대신 쓸지 한 줄씩 명시했다. blast-radius는 안 고친 코드이고 diff-radius는 이미 고친 변경, pr은 만들기이고 review는 검토, setup은 최초 1회이고 build-graph는 그래프만.

## [0.42.1] - 2026-07-26

### Fixed

- **인터뷰 Continuity 모순 감지가 항상 false였다** — 존재하지 않는 차원명을 비교하고 있었다. `contradictions` 배열 존재 여부로 바꾸고 감지된 모순을 세션과 이벤트에 남겨 라운드별로 조회할 수 있게 했다.
- **코드 그래프 증분 빌드에서 역방향 엣지 유실** — 파일 F만 바뀌면 F를 참조하는 A가 재파싱 대상에서 빠져, `deleteByFile`이 지운 A→F 엣지가 복원되지 않았다. 변경 파일을 참조하는 파일 목록(1-hop)을 함께 재파싱한다.
- **Drift 감지 상시 발화** — 목표와 산출물 문장 사이 Jaccard 유사도가 구조적으로 낮게 나와, 정렬된 산출물조차 임계값을 넘겨 CRITICAL로 오판정됐다. 임베딩 코사인 유사도로 바꾸고(임베딩 실패 시 Jaccard 폴백) 임계값을 0.3에서 0.6으로 올렸다. 임베딩 호출이 비동기라 execute와 benchmark 호출 체인 전체에 async가 전파된다.
- **세션 replay가 `completedTaskIds`를 복원하지 않았다** — 서버 재시작 후 진행률이 0%로 잘못 표시됐다. `evolve_fix`가 이벤트 없이 세션을 직접 바꿔 replay가 라이브 상태와 어긋나던 것도 함께 고쳤다. 과거 이벤트 로그는 마이그레이션하지 않아 신규 세션부터 적용된다.
- **텍스트 기반 스펙 생성의 해상도 하드코딩** — `resolutionScore`를 무조건 1로 박아 "완벽 해상도" 스펙으로 오기록됐다. null(명시적 미채점)로 바꿨다.
- **`force: true` 우회에 감사 기록이 없었다** — `ges_generate_spec`을 `force: true`로 호출해 임계값을 넘길 때 `SPEC_FORCE_OVERRIDE` 이벤트를 남긴다.

## [0.42.0] - 2026-07-25

### Added

- **`presentation-writer` 에이전트와 `presentation` 스킬** — 슬라이드별 메시지와 카피, 데이터 요약, 발표 노트는 writer가 쓰고 Reveal.js 구조와 비주얼은 기존 presentation-designer가 맡아 역할을 나눴다. 미니 인터뷰로 주제와 청중, 분량을 확정하고 승인 단계를 거쳐 HTML까지 간다.

### Fixed

- **`diff_radius`가 MCP 클라이언트에 광고되지 않았다** — `server.ts`에 등록된 입력 스키마가 `schemas.ts`의 `codeGraphInputSchema`와 어긋나 액션과 `diffMode` 파라미터가 빠져 있었다.

### Documentation

- **문서의 개수와 목록을 실제 코드에 맞췄다** — 설치 안내 표에 "13 Role+3 Review"라고 적혀 있었는데 실제는 9 Role에 4 Review였다. MCP 도구 목록, `src/` 디렉토리, 아키텍처 다이어그램도 함께 정합화했다.
- **누락된 MCP 도구 문서화** — `ges_graph_visualize`, `ges_generate_kb`, `ges_search`, `ges_sync` 섹션을 새로 썼다.
