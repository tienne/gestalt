...
- **영어 관용 비유를 한국어 명사로 직역한 자리에 `F-11`을 세웠어요.** 리뷰 답글에 "남은 r: 2건은 뿌리가 하나예요"라고 쓴 게 시작이었어요. root cause를 그대로 옮긴 말인데 F 절 어디에도 안 걸렸어요.
  - 기존 룰로는 안 잡혀요. `F-7`은 방향이 반대라 기술 어휘를 일상 자리에 끌어온 쪽만 봐요. `F-9`는 그 자리에서 지어낸 별칭을 보고 `F-10`은 동사 쪽이라 명사가 빠져나가요. `B-3`은 음차형("로우 행잉 프룻")을 잡지 그 음차를 직역한 꼴은 안 봐요
  - **가르는 기준은 비유를 걷어낸 자리를 원문 뜻 그대로 채울 한국어 표현이 있느냐예요.** 처음엔 "명사가 있느냐"로 뒀는데 리뷰에서 자기 예시가 그 기준으로 떨어진다는 걸 잡았어요 — "낮은 가지 열매"를 대신할 한국어 **명사**는 없고 "금방 되는 일"이라는 표현이 있을 뿐이에요. 예시 셋에 대체어를 짝지어 붙였어요
  - **자료구조나 명세에서 이름으로 굳은 직역은 예외예요.** 이것도 리뷰가 잡았어요. 레포 산문에 트리의 root를 부르는 "스레드의 뿌리"와 "ref의 뿌리"가 11곳 있는데, 룰이 열어둔 형태 예외가 "뿌리 깊은"과 "뿌리내리다" 둘뿐이라 전부 위반이 될 뻔했어요. 룰이 예상하고 남긴 백로그가 아니라 예외 조항이 존재를 고려 안 한 부류였어요
  - **`F-9` 예외보다 이 룰이 먼저예요.** `F-9`가 "명세나 교과서에 뜻이 박힌 말"을 면제하는데 그게 `F-11` 대표 예시 "은탄환"을 그대로 통과시켜요. Brooks 논문에 실린 용어거든요. 원본이 비유면 `F-11`이 본다고 정하고 `F-9` 행에도 같은 문장을 넣었어요 — 이 룰북은 인접 룰 경계를 늘 양쪽에서 적어요
  - **`F-7`과 겹치면 `F-7` 하나로 판정해요.** 촉매나 임계질량처럼 이공계 어휘가 영어 관용구로도 굳은 말이 그 자리예요. `F-10`이 `F-8`을 두고 한 것과 같은 방식이에요
  - **팀에서 굳었다는 건 예외가 아니에요** — `F-9`와 같은 쪽이고 `B-4`와는 달라요. 사전에 관용구로 올라 있거나 영어를 모르는 한국어 화자가 그 뜻으로 쓰면 예외예요
  - **대화와 리뷰에서 `S1`이에요.** `A-20`을 `S2`로 남긴 것과 갈리는 자리인데, 정당한 쓰임이 자료구조 이름 말고는 거의 없어서예요. 그 예외는 위에서 형태가 아니라 뜻으로 잘라뒀어요
  - **탐지기는 안 붙였어요.** "뿌리 깊은"과 "뿌리내리다"가 정상 한국어라 형태로는 못 갈라요. 다만 원인 뜻이 형태로 드러난 조합("뿌리가 같다", "같은 뿌리")은 `verify-rule-refs.ts`의 `COINED_TERMS`에 넣어 문서 산문에서 잡아요. 룰 전체를 거는 게 아니라 확실한 자리만 거는 거예요
  - 실제 예문에서 옳게 판정되는지 볼 자동 테스트는 없어요. `F-9`, `C-14`, `G-4`가 이미 같은 공백을 안고 있고 이번에도 그대로 물려받아요

### Changed

- **`build-output-style.ts`의 `GATE` 주석에 편입 기준을 적었어요.** `SPOTLIGHT`가 32개, 게이트가 19개까지 늘었는데 룰을 더할 때 항상 켜지는 자리에 넣을지 룰북에만 둘지를 고르는 기준이 어디에도 없었어요. 이 목록은 매 세션 비용이라 자동으로 따라오면 안 돼요.
