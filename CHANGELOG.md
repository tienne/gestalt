...
- **명사구로 끊는 자리에 룰 셋을 세웠어요.** "두 가지 짚을 것" 한 문장에 서로 다른 세 가지가 겹쳐 있었어요. 하나로 뭉치면 예시 하나만 잡고 나머지 자리는 못 잡아요.
  - `C-14` 수량 예고 — 개수를 앞세워 문단을 여는 꼴이에요. 개수가 대상의 고정 속성이면 그냥 사실이라 대상이 아니에요("다섯 가지 게슈탈트 원리")
  - `E-8` 명사구 종결 — 서술어 없이 관형형과 의존명사로 끊는 꼴이에요. 헤딩과 표 셀, 체크리스트, 규범 문서의 볼드 도입구는 예외예요
  - `G-4` 화자 소거 — 의견인데 화자가 없어 사실 선언으로 읽히는 자리예요. `G-2`와 방향이 반대라 경계를 적어뒀어요
- **셋 다 탐지기를 안 붙였어요.** 뜻을 봐야 갈려서예요. `C-14` 후보 정규식으로 레포를 훑어보니 22개 중 절반 넘게 개수가 대상의 고정 속성인 사실 서술이었어요. `detectors.ts`는 재현율을 올리는 도구가 아니라 잘못 걸리는 게 없는 회귀 게이트예요.

### Changed

- **`I-5`의 "건"과 "개" 경계를 다시 그었어요.** 세 번 고쳐 자리를 잡았어요.
  - 1차 "안건을 넣어 말이 되는가" — 관형절을 어떻게 붙이느냐에 좌우돼 같은 문서 안에서 다른 답이 나왔어요
  - 2차 "세는 사물이 명사로 드러났는가" — 지시 대상이 아니라 표현에 걸려 고쳐쓰기로 우회됐어요
  - 3차 판정 범위를 좁혔어요. 사물이 바로 앞에 적힌 자리("버그 8건")만 봐요. 무엇을 세는지 문장에 없으면("확인이 필요한 게 2건 있습니다") 판정하지 않아요. 우회할 기준 자체가 없어져요
  - 집계 표기는 형태가 아니라 **적용 범위**로 뺐어요. `I-7`이 이미 갖고 있는 "산출물 문장에만 적용된다"와 같은 꼴이에요
- **`B-5`를 다의어 동사까지 넓혔어요.** 원래 명사만 겨냥하던 룰이에요. `close`는 자리마다 갈리는데(코멘트는 끝내다, 이슈는 종료하다, 연결은 끊다) "닫다" 하나로 몰려 있었어요. `leak`도 누출, 미탐, 빈발 세 뜻에 몰려 있어 누출일 때만 남겼어요.
  - 작업 도중 "닫다"를 "맺다"로 일괄 치환했다가 그것도 만능 동사를 새로 만드는 거라 룰 본문에 그 사실을 적어뒀어요
  - 스윕은 마크다운에서 시작해 CLI 도움말, MCP 도구 설명, 실행 출력까지 갔어요. 상태를 가리키는 "닫힌 PR"은 `CLOSED` 상태값을 옮긴 말이라 대상에서 뺐어요
- **`output-style`이 금지 29개, 게이트 16개로 늘었어요.** 새 룰 셋이 `GATE`와 `SPOTLIGHT`에 들어갔어요.
- **측정 근거 절의 출처 링크를 걷었어요.** 대조 코퍼스 수치와 조정 근거는 그대로 있어요.

### Fixed

- **`I-5`가 탐지기 동작을 틀리게 서술하고 있었어요.** "탐지기는 수량 자리를 안 본다"고 적었는데 실측하면 한글 한 글자에 조사가 붙은 꼴("여섯 건은", "두 건이")이 걸려요. 룰 문서가 탐지기를 틀리게 적으면 모델이 탐지기 결과를 잘못 감지로 무시해요.
- **주석 두 자리가 코드와 어긋났어요.** `review-passthrough.ts`가 스레드 `resolve`를 "종료"라고 적어 PR 닫기로 읽혔어요. `detectors.ts`는 잘못 감지를 적는 자리에 미탐을 적었고요.
- **룰북과 `author-voice`가 서로를 되돌리던 자리를 막았어요.** `E-8`이 내놓는 교정 결과를 `G-4`가 위반 예시로 들었어요. 볼드 도입구 셋은 자기가 세운 `E-8`을 어기고 있었고요.

### 검증 범위

로컬 리뷰를 5라운드 돌렸어요. 이슈가 20건에서 1건으로 줄었고 critical은 라운드 1에서만 나왔어요. 남은 하나는 내부 주석의 close 잔여라 사용자에게 안 보여요. `pnpm verify:rules`가 세 번 걸렸는데 세 번 다 새로 쓴 설명 문장이 `C-11`을 어긴 자리였어요 — 룰북이 자기 규칙으로 자기를 검사하는 구조가 잡아준 거예요.

### 남긴 것

- `SPOTLIGHT` 역방향 검사 — 대화 S1인데 목록에 없는 룰을 아무도 안 잡아요
- `policy.ts`와 `repository.ts` 내부 주석의 close 잔여
- `references/scholarship.md` 부재 — 룰북이 참조하는데 파일이 없어요. 이번 변경 이전부터 있던 문제예요

## [0.72.2] - 2026-08-30

### Fixed

- **npx 기동 실패로 끊기던 MCP 연결을 런처로 잡았어요.** npx는 버전을 정확히 박아도 기동할 때마다 레지스트리를 조회해요. 캐시가 warm이면 1.9초지만 비면 20초, 레지스트리에 못 닿으면 70초를 매달린 뒤 실패해요. Claude Code가 stdio 서버에 주는 기동 제한은 30초라 콜드 상태와 오프라인 상태가 둘 다 `Connection closed`로 나타났어요.
  - GUI로 띄운 세션은 더 빨리 죽었어요. 데스크톱 앱이 물려주는 PATH에는 버전 매니저가 없어서 npx를 아예 못 찾고 0초에 끝났어요
  - `scripts/mcp-serve.sh`가 셋을 처리해요. nvm, fnm, Volta, Homebrew에서 Node >= 20을 찾아요. 전역 gestalt가 있으면 그걸 쓰고 없으면 `npx --offline`으로 캐시에서 해석해요
  - npx로 서버를 띄우지 않고 bin 경로만 받아와 직접 exec해요. npx가 cwd의 로컬 패키지를 먼저 보기 때문에 `node_modules` 없는 gestalt 체크아웃 안에서는 command not found로 죽어서예요. 그래서 해석은 `cd /`에서 해요
  - 측정값(initialize 응답까지) — 터미널 warm 1.9초 → 1.5초, GUI 최소 PATH 즉시 실패 → 1.5초, gestalt 레포 안 즉시 실패 → 2.1초
- **Codex와 Grok 매니페스트의 npx 스펙을 버전으로 핀했어요.** 버전 없는 스펙은 npx가 매번 npm의 latest를 끌어와요. 그래서 플러그인 캐시는 0.72.0인데 서버만 0.72.1로 도는 조합이 실제로 만들어져 있었어요. 번들된 스킬과 서버가 어긋나요. `sync-version.ts`가 릴리즈마다 이 핀을 갱신해요.

## [0.72.1] - 2026-08-29

### Changed

- **"정본"과 "표면"을 걷었어요.** 둘 다 단어 하나로 여러 뜻을 뭉개고 있었어요. 그 자리마다 무엇을 가리키는지는 이미 정해져 있었는데도요. `style-guide.md`가 "첫 번째 선택지는 단어를 안 붙이는 것"이라고 적어두고도 정작 그 표 안에 뭉개는 단어가 있었고요.
  - `정본` — 여섯 자리 중 넷은 사본 대비 원본이 아니라 그냥 "이게 기준이다"였어요. "본문이 정본입니다"는 한 번 더 옮겨야 읽히는데 "어긋나면 본문을 따릅니다"는 그대로 행동이에요
  - `표면` — 한 단어에 세 뜻이 얹혀 있었어요. 인터페이스(CLI와 MCP와 웹), 사용자 노출, 탐지 범위예요. 자리마다 대상 이름을 부르거나 "호출하는 쪽"으로 풀었어요
- **`surface`와 `deep`을 음차표에 올렸어요.** `gate`와 `layer`가 이미 정리돼 있던 그 표예요. 이번에 자리에 따라 아홉 갈래로 옮긴 판단을 그대로 적었어요. 다음에 같은 말을 쓸 때 처음부터 다시 판단하지 않아도 돼요.
- **음차표의 적용 범위를 못박았어요.** 세 가지는 대상이 아니에요.
  - 코드 식별자 (`sanitizeSurfaceContext`, `DEEP_PROMPT_KEYS`)
  - 그 도메인의 정착 용어 — 디자인 시스템의 `surface`와 `onSurface`(Material Design 색 토큰), CI의 gate, API gateway가 그래요. **이 파일은 플러그인으로 배포돼 다른 레포에서도 읽히므로** 프론트엔드 레포에서 색 토큰을 "표면"으로 옮기라는 말이 되면 안 돼요
  - 이미 나간 CHANGELOG와 릴리즈 노트
- **"대비쌍은 짝을 함께 옮긴다"를 적었어요.** "표면"만 걷었다가 짝인 "심층"이 여섯 곳에 남아 무엇의 반대인지 가리킬 데를 잃은 게 이번 사례예요. `사용자 노출 ↔ 게슈탈트 내부`로 함께 세웠어요.

### Fixed

- **`local-pr-web/engine.ts`의 주석이 틀린 걸 가리키고 있었어요.** "CLI와 웹이 서버를 공유"라고 적혔는데 그 파일이 웹 엔진 자체라 웹이 자기 자신과 공유한다는 말이었어요. 실제 시나리오는 CLI와 MCP예요.
- **`docs/local-pr.md`가 "코멘트 작성과 판정은 CLI 몫"이라고 적었어요.** `ges_pr`에도 `comment`와 `resolve`와 `review`가 있어요. 대비 대상은 원래 읽기 전용인 웹 UI였고요.

### 검증 범위

로컬 리뷰를 6라운드 돌렸어요. 이슈가 6건에서 0건으로 줄었고 blocking은 라운드 2부터 0이었어요. 라운드 1에서 정합 심급이 escalate를 냈고 두 갈래였는데 위의 대비쌍과 음차표가 그 대응이에요.

리뷰가 잡아낸 것 중 셋은 이번 작업이 새로 만든 오류였어요 — `engine.ts`가 원문보다 나빠진 자리, `pr.ts`가 한정을 잃어 CLI가 유일한 진입점처럼 읽히던 자리, `README.ko.md`에서 서술어만 바꾸고 주어를 안 세운 자리예요. 마지막 건 여섯 번째 라운드에서야 잡혔어요.
...
- **`humanize-scan`이 맞춤법을 따로 세어 줘요.** 커밋 해시 뒤 조사와 외래어에 붙는 "하다"의 띄어쓰기를 봐요. 어투가 아니라 맞춤법이라 S1 총계와 윤문 등급에는 안 섞고 별도 절로 내놔요. 어투는 깨끗한데 맞춤법만 걸린 원고는 종료 코드 11로 답해요 — 10(깨끗함)으로 닫으면 "윤문하지 않는다"로 읽혀서 그대로 나가거든요.
- **`humanize-scan`에 입력 상한이 생겼어요.** 2MB를 넘거나 파일이 아니면 읽기 전에 돌려보내요.
- **절 참조 표기가 규칙이 되고 검사에 걸려요.** 마크다운끼리는 앵커 링크(`[측정 근거](ai-tell-quick-rules.md#측정-근거)`), 소스 주석은 `파일.md §「제목 전문」`으로 씁니다. `pnpm verify:rules` 8번이 앵커와 제목이 실제로 있는지 봐요. 그동안 `§`는 아무도 검사하지 않아서 제목을 고쳐도 참조만 낡았어요 — 기존 참조 열 곳 중 일곱 곳이 이미 어긋나 있었어요. README의 죽은 앵커도 이 검사가 처음 잡았고요. 규칙은 `style-guide.md`의 "다른 문서의 절을 가리킬 때"에 적어뒀어요.

### Changed

- **`I-6`, `I-7`, `F-8`이 놓치던 형태를 잡아요.** 피동 `산출된다`, 존댓말 `산출합니다`와 `산출됩니다`, 조사가 붙은 `산출도`, 주어를 안 보고 잡는 `지적했던 부분`, 진행형 `재보고 있다`, 관형형 `재본 결과`예요. 셋 다 룰이 말하는 자리인데 첫 판 정규식이 못 보던 곳이에요.
- **레포 안에서 룰이 막는 말을 걷었어요.** 테스트 제목과 픽스처의 `지적`을 `의견`으로, 주석의 `오탐`을 `잘못 감지`로 바꿨습니다. 검사가 강제한 건 아니고 룰을 레포에 되먹인 자리예요.
...
- **output style이 새 룰 넷을 같이 실어요.** `pnpm build:output-style`로 다시 뽑으면 금지 26개에 자가점검 13개예요.
