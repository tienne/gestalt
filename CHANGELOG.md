...
- **`gestalt review-loop` CLI.** 리뷰 루프가 판정에 쓰는 수를 내는 명령이에요. `state`는 PR 상태와 스레드를 한 번에 받아 미대응 수와 다음에 할 일(`signal`)까지 냅니다. `dir`은 라운드 상태를 두는 자리를 내고 `parse`는 번호나 URL에서 PR 번호와 레포를 읽습니다.
  ...
  - `signal`은 `CLOSED`, `REREVIEW_REQUESTED`, `WAITING`, `READY`, `REPLIES_ONLY` 다섯이에요. PR 상태와 미대응 수, 새 커밋 여부, 재리뷰 요청 네 값의 곱집합이 전부 어느 하나로 갑니다.
  ...
  - 상태 자리도 레포까지 넣어 가릅니다. `dir`은 뿌리만 내고 PR별 자리는 `state`가 `stateDir`로 함께 내요 — 대상 문자열을 셸에 안 넘기려면 파일로 떨궈야 하는데, 그 파일을 PR별 자리에 두면 자리 이름에 필요한 번호를 아직 모르는 단계라 순환이 생깁니다.
