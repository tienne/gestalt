...
- **`F-9`가 원인 관계를 비유로 뭉갠 자리도 봐요.** 리뷰 답글에 "남은 r: 2건은 뿌리가 하나예요"라고 쓴 게 시작이었어요. 원인이 같다는 말을 뿌리에 얹으니 무엇이 같은지가 안 읽혀요.
  - 처음엔 `F-11`을 따로 세웠어요. 영어 관용 비유를 직역으로 옮긴 자리를 잡으려던 룰인데 두 라운드 리뷰를 거쳐 접었어요. 룰의 기준이 어원이면 판정이 어원 판정이 되는데, "뿌리"의 근본 뜻은 표준국어대사전에 비유 뜻풀이로 실려 있어서 root cause를 옮긴 말인지 한국어 자체의 말인지를 쓰는 사람이 그 자리에서 확인할 수 없어요
