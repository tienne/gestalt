...
- **"정본"과 "표면"을 걷었어요.** 둘 다 단어 하나로 여러 뜻을 뭉개고 있었고, 그 자리마다 무엇을 가리키는지는 이미 정해져 있었어요. `style-guide.md`가 "첫 번째 선택지는 단어를 안 붙이는 것"이라고 적어두고도 정작 그 표 안에 뭉개는 단어가 있었고요.
  - `정본` — 여섯 자리 중 넷은 사본 대비 원본이 아니라 그냥 "이게 기준이다"였어요. "본문이 정본입니다"는 한 번 더 옮겨야 읽히는데 "어긋나면 본문을 따릅니다"는 그대로 행동이에요
  - `표면` — 한 단어에 세 뜻이 얹혀 있었어요. 인터페이스(CLI·MCP·웹), 사용자 노출, 탐지 범위예요. 자리마다 대상 이름을 부르거나 "호출하는 쪽"으로 풀었어요
- **`surface`와 `deep`을 음차표에 올렸어요.** `gate`와 `layer`가 이미 정리돼 있던 그 표예요. 이번에 자리에 따라 아홉 갈래로 옮긴 판단을 그대로 적어서, 다음에 같은 말을 쓸 때 처음부터 다시 판단하지 않아도 돼요.
- **음차표의 적용 범위를 못박았어요.** 세 가지는 대상이 아니에요.
  - 코드 식별자 (`sanitizeSurfaceContext`, `DEEP_PROMPT_KEYS`)
  - 그 도메인의 정착 용어 — 디자인 시스템의 `surface`와 `onSurface`(Material Design 색 토큰), CI의 gate, API gateway예요. **이 파일은 플러그인으로 배포돼 다른 레포에서도 읽히므로** 프론트엔드 레포에서 색 토큰을 "표면"으로 옮기라는 말이 되면 안 돼요
