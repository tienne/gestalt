...
- **`gestalt review-loop` CLI.** 리뷰 루프가 판정에 쓰는 수를 내는 명령이에요. `state`는 PR 상태와 스레드를 한 번에 받아 미대응 수와 다음에 할 일(`signal`)까지 내고, `dir`은 라운드 상태를 두는 자리를, `parse`는 번호나 URL에서 PR 번호를 냅니다.
  - 이걸 스킬 문서에 셸로 적던 때는 코드블록이 각각 다른 Bash 호출이라 조회와 집계 사이를 파일로 이어야 했어요. 스냅샷이 이번 조회의 것인지 가리는 표식, 그 표식의 신선도 검사, 블록마다 좌표를 다시 세우는 관용구가 전부 거기서 나왔고 — 표식을 쓰는 경로와 읽는 경로가 갈려 판정 직전에 멈추는 자리도 함께 나왔습니다. 한 프로세스 안에서 조회하고 세면 그 중간이 없어요.
