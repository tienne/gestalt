...
- **`F-10`의 형태 예외에 피동 "박히다"를 넣었어요.** 그 행이 "형태로 빠지는 예외는 앞에 못이 없는 박아두다와 명사 잠금뿐이다"라고 적어뒀는데, 탐지기 정규식이 `못` 접두를 요구해서 "명세에 뜻이 박힌 말"도 실제로는 안 걸립니다. 선언이 탐지기보다 좁았어요.
  - **형태 예외에만 넣고 판정 예외 목록은 셋 그대로 뒀어요.** 판정 예외는 사람이 "누가 정하느냐" 축으로 다시 보는 자리인데, 피동을 거기 넣으려던 근거가 두 번 걷혔습니다. 처음엔 "뜻을 정한 주체가 명세라 사람 쪽"이라고 적었는데 명세는 문서라 축이 반대 답을 냈고요. 다음엔 "피동이라 주체가 안 서니 축이 적용 안 된다"고 적었는데, 그 셀은 이미 "주어가 없으면 목적어로 본다"는 폴백을 갖고 있어서 축 밖이 아니었어요.
  - 그 근거에는 `못` 제한도 없어서 "규칙이 못 박혀 있다"까지 예외로 빠져나갔습니다. 그건 이 행의 헤더 예시를 피동으로 바꾼 꼴이고 탐지기는 실제로 잡아요. 선언이 탐지기보다 좁던 걸 고치려다 반대로 넓혀놓은 자리였습니다.
  - 탐지 범위는 안 넓어졌어요. 앞에 `못`이 없는 `박히다`는 선언 전에도 안 걸렸습니다. `못`이 붙은 피동("규칙이 못 박혀 있다")은 그대로 걸리고 사람이 축으로 다시 봐요. 선언을 탐지기 실제 동작에 맞춘 거예요.
  - `rule-check-scope` 테스트에 두 케이스를 넣고 회귀 코퍼스에도 셋을 넣었어요. 탐지기에 박히다류 분기가 없다는 것, 실물 잠금은 형태로 안 빠지고 걸린 뒤 사람이 가른다는 것, 그리고 `못`이 붙은 피동은 걸린다는 것입니다. 실물 잠금 쪽은 "사람 사이 합의" 사례와 대칭인데 그동안 비어 있었어요.
  - `src/humanize/detectors.ts` 주석도 형태 예외를 하나로 닫아 뒀던 걸 함께 고쳤습니다.
