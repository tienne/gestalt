...
Gestalt는 **MCP 서버**로 실행돼요. Claude Code나 Codex 같은 호스트 코딩 에이전트를 통해 사용할 경우 호스트가 LLM 역할을 담당해요 — Gestalt는 프롬프트와 컨텍스트를 반환하고, 실제 추론은 호스트가 해요.

> **참고해 주세요** — Execute 단계는 API 키 설정 여부와 무관하게 **항상 Passthrough 모드**로 동작해요. Execute는 호스트의 도구(Bash, Edit 등)로 실제 파일 수정과 코드 실행을 수행하기 때문에, 호스트가 LLM 주체가 되는 게 설계 의도예요. 자체 LLM으로 대체하는 모드는 따로 없어요(미구현이 아니라 설계상 그래요).
...
Grok Build TUI/CLI용으로 MCP 서버랑 워크플로 스킬을 한 번에 받아요. 마켓플레이스는 `.grok-plugin/marketplace.json`을 읽어요. Claude 마켓플레이스는 레포 루트를 복사하니 여기서 쓰지 마세요.
...
> Execute는 **항상 Passthrough 모드**로 동작해요 — 실제 파일 수정과 코드 실행은 호스트 코딩 에이전트가 도구(Bash, Edit 등)로 수행해요. API 키를 설정해도 Execute가 자체 LLM 모드로 바뀌지 않아요. 그런 모드는 설계상 존재하지 않아요.
