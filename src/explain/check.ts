 ...
 *
 * 한 갈래를 넣을 때 활용형을 함께 적는다. '라고 생각하면'만 있던 동안 "여러 사람이 같은
 * 공책에 동시에 적는다고 생각해보세요"가 outsider 에서 채택 금지를 받았다. 같은 대입
 * 갈래인데 활용형 하나만 목록에 있어서다.
 *
 * **활용형을 어간까지만 자르지 않는다.** '생각해보'와 '떠올려'와 '상상해'까지만 적어 보니
 * "곰곰이 생각해보니", "상상해 본 적 없다" 같은 비유 없는 문장이 전부 통과했다. 셋 다
 * 비유 갈래에 묶인 말이 아니라 한국어 산문에 흔한 동사 어간이라서다. outsider 와 nontech
 * 에게 이 축은 유일한 필수 관문이라(coverage 는 off, grounding 은 warn 까지) 그 바닥이
 * 0 으로 내려갔다.
 *
 * 대입은 인용형 '-다고'와 '-라고'까지 적는다. '고'까지만 적으면 그것도 모자란다 — 어떤
 * 연결어미든 뒤에 붙으므로 "밥을 먹고 생각해보니"가 그대로 통과한다. 그려보기는 보조용언
 * '보'를 요구한다. 어느 쪽이든 앞뒤로 자기 갈래를 가리키는 말이 하나는 붙어 있어야 한다.
 *
 * 띄어쓰기는 두 꼴을 다 적는다 — 한글 맞춤법이 보조용언 띄어쓰기를 양쪽 다 허용한다.
 */
