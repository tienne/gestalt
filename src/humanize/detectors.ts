...
const DETECTORS: Detector[] = [
  ...
  // 활용형은 한 글자씩 적는다. 한글은 자모가 결합해서 "하"로는 "한다"를 못 덮는다
  matcher(
    'I-6',
    /실측|계측|오탐|산출(?=하|한|했|해|되|된|될|됐|[으은는이을과와에도만]|로|까지|부터)/g,
  ),
  // 리뷰 코멘트를 가리키는 호칭만 본다 — 내 것은 "남겼던 의견", 상대 것은 "짚어주신 부분".
  // 여섯 갈래다: 조사, 상대가 남겼음을 드러내는 활용형, "지적 N건", "지적 사항",
  // "지적들", 그리고 내가 남긴 것을 가리키는 "지적했던 부분" 꼴.
  // 마지막 갈래는 뒤에 오는 명사로 묶는다. 활용형을 지적하-까지 그냥 열면
  // "원칙 위반을 지적하고" 같은 일반 동사 용법이 걸린다
  matcher(
    'I-7',
    new RegExp(
      `지적\\s*(?:을|이|은|도|만|에|의|과|와)${NOT_HANGUL}|지적(?:해\\s*주|하신|받|당)|지적\\s*\\d+\\s*건|지적\\s*사항|지적들|지적(?:했던|한)\\s*(?:부분|내용|코멘트|의견|점|사항|건)`,
      'g',
    ),
  ),
