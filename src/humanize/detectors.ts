 ...
 * 룰 ID별 결정적 탐지기.
 *
 * 여기 있는 건 정규식만으로 멀쩡한 문장을 잘못 감지하는 일 없이 셀 수 있는 룰뿐이다.
 * D-5 의인화 주어처럼 뜻을 봐야 판단되는 룰은 일부러 뺐다 — 멀쩡한 문장에서 검사가
 * 멈추면 아무도 안 쓴다.
...
const DETECTORS: Detector[] = [
  ...
  // "논문이 지적했던 부분"은 잘못 감지한 채로 남는다. 앞에 화자가 있는 갈래를 따로 둬서
  ...
  //
  // 사이 토큰은 셋까지 허용한다 — "제가 리뷰에서 여러 번 지적한 부분"이 그만큼 벌어진다.
  // 대신 그 토큰에서 문장부호를 뺀다. 이, 가, 은, 는으로 끝나는 토큰도 뺀다.
  // 노린 건 3인칭 주어다. 안 빼면 창이 절 경계를 넘어 "제가 보기에 그 논문이 지적한
  // 부분"의 주어를 통째로 삼킨다. 그러면 이 갈래가 존재하는 이유가 없어진다.
  //
  ...
  // 두 한계 다 코퍼스가 고정한다. 놓치는 쪽은 miss 의 "I-7 한계" 라벨에 있다.
  // 잡는 쪽은 hit 케이스에 라벨 자리가 없어 "제가 보기에 논문 저자 지적한 부분" 문장 자체다.
  // 그 문장은 잡아야 할 자리가 아니라 못 가르는 오탐이다. 창을 건드리면 둘 다 깨진다.
  //
  ...
  matcher(
    'I-7',
    new RegExp(
      `지적\\s*(?:을|이|은|도|만|에|의|과|와)${NOT_HANGUL}|지적(?:해\\s*주|하신|받|당)|지적\\s*\\d+\\s*건|지적\\s*사항|지적들|(?<![가-힣])(?:제가|내가)\\s*(?:(?![^\\s.,、!?。]*[이가은는]\\s)[^\\s.,、!?。]+\\s+){0,3}지적(?:한|했던|했었던)\\s*(?:부분|점|내용|사항|코멘트|의견|건)|지적했던\\s*(?:부분|내용|코멘트|의견)`,
      'g',
    ),
  ),
