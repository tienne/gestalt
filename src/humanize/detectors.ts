...
const DETECTORS: Detector[] = [
  ...
  // 같은 줄만 본다. 개행까지 넘으면 앞 문단이 이, 가, 은, 는으로 끝났을 때
  // 다음 문단 첫 문장이 통째로 빠진다.
  ...
  matcher(
    'I-7',
    new RegExp(
      `지적\\s*(?:을|이|은|도|만|에|의|과|와)${NOT_HANGUL}|지적(?:해\\s*주|하신|받|당)|지적\\s*\\d+\\s*건|지적\\s*사항|지적들|(?<![가-힣])(?:제가|내가|우리가|저희가|저는|나는|우리는|저희는)\\s*(?:(?![^\\s.,、!?。:;()…]{0,20}[이가은는]\\s)[^\\s.,、!?。:;()…]{1,20}\\s+){0,3}지적(?:한|했던|했었던)\\s*(?:부분|점|내용|사항|코멘트|의견|건)|(?<![가-힣][이가은는][ \\t]*)지적했(?:었)?던\\s*(?:부분|내용|코멘트|의견)`,
      'g',
    ),
  ),
