...
const DETECTORS: Detector[] = [
  ...
  //
  // 그 lookbehind 는 줄바꿈 하나까지만 넘는다. 개행에는 문단 경계와 소프트 랩이 섞여
  ...
  matcher(
    'I-7',
    new RegExp(
      `지적\\s*(?:을|이|은|도|만|에|의|과|와)${NOT_HANGUL}|지적(?:해\\s*주|하신|받|당)|지적\\s*\\d+\\s*건|지적\\s*사항|지적들|(?<![가-힣])(?:제가|내가|우리가|저희가|저는|나는|우리는|저희는)\\s*(?:(?![^\\s.,、!?。:;()…]{0,20}[이가은는]\\s)[^\\s.,、!?。:;()…]{1,20}\\s+){0,3}지적(?:한|했던|했었던)\\s*(?:부분|점|내용|사항|코멘트|의견|건)|(?<![가-힣][이가은는][ \\t]*\\n?[ \\t]*)지적했(?:었)?던\\s*(?:부분|내용|코멘트|의견)`,
      'g',
    ),
  ),
