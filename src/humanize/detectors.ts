...
const DETECTORS: Detector[] = [
  ...
  // 늘릴 때는 코퍼스도 함께 늘린다. 산출물, 산출량, 산출식, 산출 경로는 그 자리에
  // 다른 글자가 와서 빠진다 — F-7의 "잠금 파일"과 같은 기준이다.
  //
  // 한 글자 조사에는 NOT_HANGUL 을 붙인다. 그게 없으면 조사가 아니라 합성어의 첫
  // 글자인 자리("산출도구", "산출과정", "산출로직")까지 걸린다. 에서, 까지, 부터는
  // 그 자체로 조사라 가드가 필요 없다. 붙이면 오히려 "산출에서"를 놓친다.
  // 활용형은 한 글자씩 적는다. "하"와 "한"은 다른 글자라 "하"를 적어도 "한다"가 안 덮인다
  matcher(
    'I-6',
    new RegExp(
      `실측|계측|오탐|산출(?=하|한|했|해|되|된|될|됐|돼|에서|까지|부터|(?:[으은는이을과와에도만]|로)${NOT_HANGUL})`,
      'g',
    ),
  ),
  // 리뷰 코멘트를 가리키는 호칭만 본다 — 내 것은 "남겼던 의견", 상대 것은 "짚어주신 부분".
  // 여섯 갈래다: 조사, 상대가 남겼음을 드러내는 활용형, "지적 N건", "지적 사항",
  // "지적들", 그리고 내가 남긴 것을 가리키는 "지적했던 부분" 꼴.
  //
  // 마지막 갈래는 "했던"만 연다. "한"까지 열면 화자 제약이 없어진다. 그러면 "논문이
  // 지적한 점", "오류를 지적한 부분"처럼 3인칭 서술이 딸려 온다. 그건 리뷰 코멘트 호칭이
  // 아니라 평범한 동사다. 같은 이유로 활용형을 지적하-까지 그냥 열지 않는다.
  // 이 룰이 산출물 문장에만 걸린다는 근거는 author-voice.md §"리뷰 코멘트를
  // '지적'이라고 부르지 않는다"의 마지막 문단에 있다
  matcher(
    'I-7',
    new RegExp(
      `지적\\s*(?:을|이|은|도|만|에|의|과|와)${NOT_HANGUL}|지적(?:해\\s*주|하신|받|당)|지적\\s*\\d+\\s*건|지적\\s*사항|지적들|지적했던\\s*(?:부분|내용|코멘트|의견)`,
      'g',
    ),
  ),
