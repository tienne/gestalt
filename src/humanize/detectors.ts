...
const DETECTORS: Detector[] = [
  ...
  //
  // 두 갈래 다 오른쪽을 막는다. 조사에는 NOT_HANGUL 을, 활용형에는 뒤따르는 어미를
  // 함께 본다. 안 막으면 조사나 어간이 아니라 합성어의 첫 글자인 자리가 걸린다 —
  // "산출도구"(도), "산출로직"(로), "산출한계"(한), "산출하위"(하), "산출해상도"(해).
  // 에서, 까지, 부터는 두 글자라 합성어 첫 글자로 오해될 일이 없어 안 막는다.
  // 그래서 그 갈래만 "산출에서는"처럼 보조사가 더 붙은 꼴을 잡는다.
  // 활용형은 한 글자씩 적는다. "하"와 "한"은 다른 글자라 "하"를 적어도 "한다"가 안 덮인다
  matcher(
    'I-6',
    new RegExp(
      `실측|계측|오탐|산출(?=하[여는지고자기며면]|한(?=다|[^가-힣])|했|해[야서도써]|되|된|될|됐|돼|에서|까지|부터|(?:[은는이을과와에도만]|로)${NOT_HANGUL})`,
      'g',
    ),
  ),
  // 리뷰 코멘트를 가리키는 호칭만 본다 — 내 것은 "남겼던 의견", 상대 것은 "짚어주신 부분".
  // 일곱 갈래다: 조사, 상대가 남겼음을 드러내는 활용형, "지적 N건", "지적 사항",
  // "지적들", 화자가 자기 것을 가리키는 "제가 지적한 부분", 주어를 생략한 "지적했던 부분".
  //
  // 활용형을 지적하-까지 그냥 열면 "원칙 위반을 지적하고" 같은 일반 동사가 걸린다.
  // 그래서 뒤에 오는 명사로 묶는데, 그것만으로는 3인칭 서술을 못 가른다 —
  // "논문이 지적했던 부분"은 오탐으로 남는다. 앞에 화자가 있는 갈래를 따로 둬서
  // "제가 지적한 점"을 되찾았다. 주어 없는 "지적했던 부분"은 뒤 명사를 넷으로 좁혀
  // 오탐 빈도만 낮췄다. 형태만으로는 여기까지가 한계다.
  // 화자와 "지적" 사이 토큰에서 문장부호를 뺀 건 안 그러면 매치가 마침표를 넘어
  // 다음 문장까지 먹기 때문이다.
  //
  ...
  matcher(
    'I-7',
    new RegExp(
      `지적\\s*(?:을|이|은|도|만|에|의|과|와)${NOT_HANGUL}|지적(?:해\\s*주|하신|받|당)|지적\\s*\\d+\\s*건|지적\\s*사항|지적들|(?:제가|내가|제)\\s*(?:[^\\s.。!?]+\\s+){0,2}지적(?:한|했던|했었던)\\s*(?:부분|점|내용|사항|코멘트|의견|건)|지적했던\\s*(?:부분|내용|코멘트|의견)`,
      'g',
    ),
  ),
