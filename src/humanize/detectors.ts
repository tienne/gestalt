 ...
 *
 * 이쪽을 다 적을 수 있는 이유는 반대쪽을 못 적기 때문이다. 산출 뒤에 붙는 파생어(산출물,
 * 산출량, 산출도구, 산출로직)는 합성 가능한 명사라 끝이 없는데, 활용 어미와 조사는
 * 한국어 형태론상 개수가 정해져 있다. 막을 것보다 열 것을 세는 쪽이 끝이 보인다.
 *
...
const DETECTORS: Detector[] = [
  ...
  //
  // close 를 옮긴 "닫다"는 목적어가 앞에 오는 자리만 본다. 어간까지만 열고 어미는 안
  // 가려서 닫는다, 닫혔다, 닫아버리면, 닫고가 전부 걸린다.
  // 목적어를 요구하는 덕에 상태를 가리키는 "닫힌 PR"과 수학에서 온 "닫힌 집합"이
  // 어순으로 빠진다 — 룰북이 대상이 아니라고 적어둔 자리가 그 둘이다.
  // 목록에서 문서, 세션, 파일, 작업을 뺀 건 에디터나 DB 연결처럼 실제로 닫는 대상과
  // 겹쳐서다. 그 자리는 표에만 두고 모델이 본다 — 미덕과 불투명이 같은 이유로 빠져 있다
  matcher(
    'B-5',
    new RegExp(
      [
        '물질화|내구성\\s*있|의미론(?!적)|돌연변이|죽임',
        '갈래(?!로\\s*(?:갈리|흩어|나뉘|나눠|묶)|별|에\\s*몰아)',
        '(?:이슈|PR|스레드|티켓|논의|리뷰|승인|판정|코멘트|발표|라운드|대화|이야기|글)(?:[를을가이도는은]|으로|로)?\\s*닫',
      ].join('|'),
      'g',
    ),
  ),
  ...
  // 예외로 적어뒀고 여기서 가르지 않는다 — 좁히려면 목적어 목록이 필요한데 그만큼 미탐이 는다
  matcher('F-10', /(?:(?<!잘)못\s?박|잠[그근갔글가긴겨겼기](?![가-힣]*파일))/g),
