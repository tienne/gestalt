 ...
 *
 * 다 무오탐은 아니다. 잘못 감지가 남는 갈래는 그 자리 주석이 어디까지 새는지 적고
 * 코퍼스가 그 경계를 잡는다.
...
const DETECTORS: Detector[] = [
  ...
  // 그래서 주어 없는 갈래에도 3인칭 주격 조사 lookbehind 를 붙였다 — "논문이 지적했던
  // 부분"은 빠지고 주어를 생략한 "지적했던 부분"은 남는다. 앞에 화자가 있는 갈래를
  // 따로 둬서 "제가 지적한 점"도 되찾았다.
  //
  ...
  // 토큰 길이도 스무 자로 묶는다. 안 묶으면 공백 없는 입력에서 각 반복이 끝까지 삼켰다
  // 되돌아와 길이의 제곱으로 늘어난다 — 4만8천 자가 2980ms 였다.
  //
  ...
  // 두 한계 다 코퍼스가 "한계" 라벨로 고정한다. 허용 범위를 건드리면 둘 다 깨진다.
  //
  ...
  matcher(
    'I-7',
    new RegExp(
      `지적\\s*(?:을|이|은|도|만|에|의|과|와)${NOT_HANGUL}|지적(?:해\\s*주|하신|받|당)|지적\\s*\\d+\\s*건|지적\\s*사항|지적들|(?<![가-힣])(?:제가|내가)\\s*(?:(?![^\\s.,、!?。:;()…]{0,20}[이가은는]\\s)[^\\s.,、!?。:;()…]{1,20}\\s+){0,3}지적(?:한|했던|했었던)\\s*(?:부분|점|내용|사항|코멘트|의견|건)|(?<![가-힣][이가]\\s)지적했[던었](?:던)?\\s*(?:부분|내용|코멘트|의견)`,
      'g',
    ),
  ),
