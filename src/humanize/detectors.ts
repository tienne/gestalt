...
const DETECTORS: Detector[] = [
  ...
  //
  // 못 박다는 어간까지만 열고 어미는 안 가린다 — "못 박았다", "못박아주세요", "못박은"이
  // 전부 걸린다. 띄어쓰기는 없어도 되고 하나까지 허용한다. 룰이 예외로 둔 자리는 못 가른다:
  // 목적어가 일정이나 범위인 사람 사이 합의도, 하드코딩을 뜻하는 "박아두다"도 여기 걸린다.
  // 뜻을 봐야 갈리는 경계라 걸린 자리는 사람이 다시 본다 — I-5가 같은 방식이다.
  // "박아두다"는 앞에 "못"이 없으면 안 걸린다. 그래서 그 예외만 형태로 빠진다.
  //
  // 자물쇠 갈래는 '잠금 파일'처럼 명사로 굳은 자리를 부정 선읽기로 뺀다. F-7에 있던
  // 규칙을 그대로 옮겼다 — 테스트나 기준을 자물쇠에 빗대는 자리가 대상이다
  matcher('F-10', /(?:못\s?박|잠[그근갔글가긴겨겼기](?![가-힣]*파일))/g),
