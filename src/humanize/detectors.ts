 ...
 * 룰 ID별 결정적 탐지기.
 *
 * 여기 있는 건 정규식만으로 잘못 걸리는 것 없이 셀 수 있는 룰뿐이다. D-5 의인화
 * 주어처럼 뜻을 봐야 판단되는 룰은 일부러 뺐다 — 멀쩡한 문장에서 검사가 멈추면
 * 아무도 안 쓴다.
 ...
 *
 * 어간이 합성어의 첫 글자로도 쓰이는 갈래는 오른쪽을 막는다. 안 막으면 "산출한계"(한),
 * "산출하위"(하), "산출해상도"(해), "산출할당량"(할), "산출도구"(도), "산출로직"(로)이
 * 걸린다. 했, 되, 된, 될, 됐, 돼는 합성어 첫 글자로 쓰일 일이 없어 안 막는다.
...
const DETECTORS: Detector[] = [
  ...
  // 미덕(virtue)과 불투명(opaque)은 한국어에서 제 뜻으로도 쓰여 잘못 걸린다. 표에만 두고 모델이 본다
  ...
  matcher(
    'F-8',
    /(?:근거|의견|판단|영향|의미|가치|리스크)(?:를|을)\s*(?:재(?=봤|봐|보니|보면|본다|보았|보고\s*있|본\s)|달아(?=[봤봐본보았뒀둔두])|달았)/g,
  ),
  ...
  // 어떤 꼴을 여는지는 정규식과 코퍼스의 I-7 항목이 갖고 있다. 여기 옮겨 적으면
  // 갈래를 하나 더 열 때마다 갈린다 — 실제로 세 라운드 연속 갈렸다.
  //
  // 활용형을 지적하-까지 그냥 열면 "원칙 위반을 지적하고" 같은 일반 동사가 걸린다.
  // 그래서 뒤에 오는 명사로 묶는데, 그것만으로는 3인칭 서술을 못 가른다 —
  // "논문이 지적했던 부분"은 잘못 걸린 채로 남는다. 앞에 화자가 있는 갈래를 따로 둬서
  // "제가 지적한 점"을 되찾았다. 주어 없는 "지적했던 부분"은 뒤 명사를 넷으로 좁혀
  // 잘못 걸리는 빈도만 낮췄다. 형태만으로는 여기까지가 한계다.
  //
  // 화자 갈래는 양쪽을 다 막는다. 왼쪽을 안 막으면 "문제가", "주제가"의 꼬리가 화자로
  // 읽힌다. 오른쪽 토큰에서 문장부호를 안 빼면 매치가 마침표를 넘어 다음 문장까지 먹는다.
  // 단독 "제"는 "실제", "규제"의 꼬리까지 먹어서 아예 뺐다. 사이 토큰을 셋까지 허용한 건
  // "제가 리뷰에서 여러 번 지적한 부분"이 한 문장 안에서 그만큼 벌어지기 때문이다.
  //
  // 이 룰이 산출물 문장에만 걸린다는 근거는 author-voice.md §"리뷰 코멘트를
  // '지적'이라고 부르지 않는다"의 마지막 문단에 있다. 정규식은 그 구분을 못 하니
  // 룰 문서에서 나는 hit 은 인용 예외로 걸러 읽는다
  matcher(
    'I-7',
    new RegExp(
      `지적\\s*(?:을|이|은|도|만|에|의|과|와)${NOT_HANGUL}|지적(?:해\\s*주|하신|받|당)|지적\\s*\\d+\\s*건|지적\\s*사항|지적들|(?<![가-힣])(?:제가|내가)\\s*(?:[^\\s.。!?]+\\s+){0,3}지적(?:한|했던|했었던)\\s*(?:부분|점|내용|사항|코멘트|의견|건)|지적했던\\s*(?:부분|내용|코멘트|의견)`,
      'g',
    ),
  ),
