 ...
 * 채택 금지이고 S2 이하는 경고다. 제거율 0은 변경까지 문턱 아래일 때만 채택 금지다.
 *
 * 잔존 축의 판정 근거는 탐지기가 있는 룰의 건수다. 제거율이 0인 자리에서만 변경률을
 * 문턱으로 함께 보는데, 그건 change-rate 축이 계산한 값을 빌려 쓰는 것이다. 그 밖에서
 * 무엇을 고쳤는지는 판정 범위에 없다. 그 자리는 사람에게 넘긴다.
 */
...
export const THRESHOLD = {
   ...
   * 뜻은 아니다 — 그건 이 검사의 판정 범위 밖이라 경고로 넘긴다.
   *
   * 변경률은 비율이라 문서가 길수록 같은 양의 교정이 낮은 값으로 나온다. 실측하면
   * 한 곳을 고칠 때 2문단에서 20%, 50문단에서 0.75%다. 그래서 문턱 아래라는 게 안
   * 고쳤다는 뜻이 아니다. 문구도 "텍스트가 그대로다"가 아니라 "변경이 문턱 아래다"이다.
   */
  idleChange: 0.05,
} as const;
 ...
 *
 * 그래서 제거율이 0일 때 여기서 확정되는 건 하나다 — 변경까지 문턱 아래면 윤문을 안
 * 한 것이다. 변경이 문턱을 넘었으면 탐지기 밖에서 고쳤을 수 있고 그건 이 코드로
 * 확인되지 않으므로 경고로 넘긴다.
 */
function checkResidualS1(
  ...
  if (removal <= 0) {
    // 확정할 수 있는 데까지만 간다. 변경이 문턱 아래면 윤문을 안 한 것이다. 문턱을
    // 넘었으면 탐지기 밖에서 고쳤을 수도 있다. 그 자리는 이 검사의 범위 밖이다.
    ...
    return {
      ...
      axis: {
        ...
        detail: idle
          ? `${scale}, 한 건도 못 줄이고 변경도 문턱 아래다 (변경률 ${pct(rate)}) — 채택 금지`
          : `${scale}, ${grew ? '탐지 가능한 룰이 오히려 늘었다' : '탐지 가능한 룰은 안 줄었다'} (변경률 ${pct(rate)}) — 탐지기 밖 룰은 이 검사로 확인되지 않는다`,
        evidence,
      },
    };
  }
