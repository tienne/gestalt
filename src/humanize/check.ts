 ...
 *
 * 각 축은 자기가 센 값으로만 판정한다. 잔존 축이 세는 건 탐지기가 있는 룰뿐이라, 그
 * 밖에서 무엇을 고쳤는지는 판정 범위에 없다. 그 자리는 사람에게 넘긴다.
 */
...
export const THRESHOLD = {
   ...
   *
   * 절대값이라 긴 문서에서는 문턱이 상대적으로 낮아진다. 그래서 문구가 "텍스트가
   * 그대로다"가 아니라 "변경이 문턱 아래다"이다. 실제로 안 바뀌었다고 단정하지 않는다.
   */
  idleChange: 0.05,
} as const;
  ...
  if (removal <= 0) {
    ...
    return {
      ...
      axis: {
        ...
        detail: idle
          ? `${scale}, 한 건도 못 줄었고 변경도 문턱 아래다 (변경률 ${pct(rate)}) — 채택 금지`
