 ...
 *
 * **알려진 접두어만 가린다.** 접두어가 없는 값(AWS 시크릿 액세스 키 같은 40자 난수)도,
 * 접두어가 있는데 아래 표에 없는 값(`glpat-`, `SG.`, `dop_v1_` 같은 것)도 못 잡는다.
 * 완전히 막는 장치가 아니라 흔한 실수를 줄이는 자리다. 표를 채우는 건 한 줄이다.
 * 경계 판정이 항목 수만큼 도므로 추가에 드는 비용도 그것뿐이다.
 *
...
const TOKEN_RULES: ReadonlyArray<readonly [prefix: string, minBody: number]> = [
  ...
  // 그대로 보내는 API 도 있다.
  //
  // 그 대가로 `Bearer authentication required` 는 `Bearer *** required` 가 된다. 여섯
  // 자를 넘는 뒷말이면 영어 단어라도 가려진다 — 짧은 말(`Bearer token`)만 살아남는다.
  // 이 파일이 세운 우선순위(못 가리는 쪽이 잘못 가리는 쪽보다 나쁘다)를 그대로 따른
  // 값이다.
  ['Bearer(?:\\s|\\\\n|\\\\r)+', 6],
