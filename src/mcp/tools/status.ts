 ...
 * 거부로 막는다.
 */
function projectRuleSourceErrors(errors: string[]): string[] {
  const shown = errors.slice(0, MAX_ERROR_LINES).map((message) => clamp(message, 200));
  const hidden = errors.length - shown.length;
  return hidden > 0 ? [...shown, `(그 밖에 ${hidden}개)`] : shown;
}
...
export function buildStatusConfigInfo(config?: GestaltConfig) {
  return {
    ...
    ruleSourceErrors: projectRuleSourceErrors(config?.ruleSourceErrors ?? []),
  };
}
