...
/**
 * Drift Detection — 태스크 실행 결과가 원래 Seed 스펙에서 얼마나 벗어났는지 측정.
 *
 * 3차원 가중합:
 *   Goal Drift (Jaccard, 50%) — 태스크 output이 Seed goal의 핵심 키워드와 얼마나 일치하는지
...
export function measureDrift(
  ...
  const goalDimension = measureGoalDrift(seed, taskResult);
  const constraintDimension = measureConstraintDrift(seed, taskResult);
  const ontologyDimension = measureOntologyDrift(seed, taskResult);
 ...
 * 높은 값 = 더 많은 drift
 */
function measureGoalDrift(seed: Seed, result: TaskExecutionResult): DriftDimension {
  const goalTokens = tokenize(seed.goal);
 ...
 */
function measureConstraintDrift(seed: Seed, result: TaskExecutionResult): DriftDimension {
  ...
  return {
    name: 'constraint',
    score: Math.round(score * 1000) / 1000,
    detail: `${violations}/${seed.constraints.length} constraints with low output overlap`,
  };
}
 ...
 */
function measureOntologyDrift(seed: Seed, result: TaskExecutionResult): DriftDimension {
  const ontologyTerms = new Set<string>();
  for (const entity of seed.ontologySchema.entities) {
    ontologyTerms.add(entity.name.toLowerCase());
    for (const attr of entity.attributes) {
      ontologyTerms.add(attr.toLowerCase());
    }
  }
  for (const rel of seed.ontologySchema.relations) {
