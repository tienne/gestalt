...
| 측량·사무투 명사 | "레포 실측으로 1444개 중", "해당 사항 확인" | 동사로 풀기("레포 뒤져보니 1444개 중") |

**근거를 제시하는 자리의 어휘.** 수치를 들이밀 때 "실측", "계측", "산출" 같은 측량투 명사를
쓰지 않는다 — 일상 대화에서 안 쓰는 말이라 그 자리만 문서 톤으로 튄다.
사람은 어떻게 알아냈는지를 동사로 말한다.
...
282건은 코퍼스 1,300건 중 20% 남짓이고, 종결어미는 실제로 여러 갈래로 흩어져 있다 —
