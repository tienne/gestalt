...
코멘트 초안을 작성한 뒤 반드시 아래 패턴을 점검하고 교정한다. `humanize-monolith` 에이전트의 S1(심각) 규칙을 준수한다.

**제거할 패턴 (S1 — 반드시 교정)**

| 패턴 | 예시 | 교정 |
|------|------|------|
| 번역투 "~를 통해" | "이 방식을 통해 해결 가능합니다" | "이 방식으로 해결할 수 있습니다" |
| 결산 피벗 | "결론적으로", "요약하자면", "정리하면" | 삭제 후 직결 |
| AI 의인화 주어 | "이 코드는 ~를 수행합니다" | "~합니다" / 주어 생략 |
| 과도한 헤징 | "~일 수 있을 것 같습니다" | "~입니다" / "~입니다" |
| 불필요한 존댓말 강조 | "~해주시면 감사하겠습니다" | "~하세요" / "~을 권장합니다" |

**유지할 패턴**
- 기술 용어(N+1, null, race condition 등)는 영어 그대로
- 수치·파일명·함수명·에러 메시지 원문 보존
- 코드 스니펫 내용 변형 금지
