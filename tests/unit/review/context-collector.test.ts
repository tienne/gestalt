...
describe('ReviewContextCollector', () => {
  ...
  describe('collectFromFiles (direct review)', () => {
    it('returns a ReviewContext from a file list without execute session', () => {
      const ctx = collector.collectFromFiles(['src/auth/jwt.ts', 'src/auth/middleware.ts']);
    ...
    it('leaves spec and taskResults undefined', () => {
      const ctx = collector.collectFromFiles(['src/a.ts']);
    ...
    it('returns changedFiles sorted', () => {
      const ctx = collector.collectFromFiles(['src/z.ts', 'src/a.ts', 'src/m.ts']);
    ...
    it('does not mutate the input array', () => {
      ...
      collector.collectFromFiles(input);
    ...
    it('never adds impacted files to the review target list', () => {
      const ctx = collector.collectFromFiles(['src/core/config.ts']);

      expect(ctx.changedFiles).toEqual(['src/core/config.ts']);
      expect(ctx.dependencyFiles).toHaveLength(0);
    });
  });
});
