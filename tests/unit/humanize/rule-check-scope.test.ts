...
describe('검사 범위와 예외', () => {
  ...
  describe('F-10 탐지기', () => {
    ...
    // 룰북 F-10 행이 넷째 예외로 선언한 피동이다. 근거는 그 행에 있다.
    // 이 정규식에는 박히다류를 잡는 분기가 없다 — 그 분기가 생기면 여기서 빨개진다
    it('탐지기에 박히다류 분기가 없다', () => {
      expect(detect('명세에 뜻이 박힌 말은 그대로 둔다', ['F-10'])).toEqual([]);
    });

    // 실물 잠금은 형태로 안 빠진다. 목적어를 안 가려 함께 걸리고 사람이 축으로 다시 본다.
    // 위 '사람 사이 합의' 사례와 대칭이다
    it('자물쇠나 문 같은 실물도 걸린 뒤 사람이 가른다', () => {
      const hit = detect('문을 잠갔다', ['F-10']).find((d) => d.ruleId === 'F-10');
      expect(hit?.count ?? 0).toBeGreaterThan(0);
    });
