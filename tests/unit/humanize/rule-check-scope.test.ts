...
describe('검사 범위와 예외', () => {
  ...
  describe('F-10 탐지기', () => {
    ...
    // 룰북 F-10 행이 예외로 선언한 피동이다. 뜻을 정한 주체가 명세라 사람이 정하는 쪽이고
    // 룰북이 제 예외를 정의할 때 쓰는 말이라, 선언과 탐지기가 갈리는지 여기서 본다
    it('명세가 뜻을 정한 자리의 피동 박히다는 안 걸린다', () => {
      for (const line of [
        '명세에 뜻이 박힌 말은 그대로 둔다',
        '교과서에 뜻이 박혀 있으면 예외다',
      ]) {
        expect(detect(line, ['F-10'])).toEqual([]);
      }
    });
