...
> 태스크가 완료됐다고 끝이 아니다. Evaluate 단계는 두 가지 관점에서 결과를 검증한다. 빌드가 깨지지 않았는가(Structural), 그리고 원래 의도에 부합하는가(Contextual).

---

## 무엇을 하는가

완료된 태스크 결과를 **2-Stage Pipeline**으로 평가한다. Structural Stage가 먼저 실행되고, 통과 시에만 Contextual Stage로 진행한다. 두 점수를 합산해 성공 여부를 판정한다.

---

## 게슈탈트 원리 적용

Evaluate는 **연속성(Continuity)** 원리가 주도한다.

실행 결과가 Spec의 흐름과 일관성을 유지하는지 측정한다. Structural은 연속성의 기술적 측면(코드가 동작하는가), Contextual은 의미적 측면(목표와 정렬되는가)을 검증한다.
...
코드가 실제로 동작하는지 자동 명령어로 검증한다.

```
lint → build → test
```

명령어는 Spec 또는 ExecutionPlan에서 파생된다. 하나라도 실패하면 **Short-Circuit** — Contextual Stage를 건너뛰고 즉시 Evolve의 Structural Fix Flow로 전달한다.
...
LLM이 Spec의 AcceptanceCriteria를 하나씩 검증한다.
...
두 조건 모두 충족 시 `status: 'success'` → 다음 단계(Code Review)로 진행.
하나라도 미달 시 Evolve 단계로 전달.
...
**왜 Structural을 먼저 하는가?**
빌드가 깨진 상태에서 LLM에게 코드를 평가시키는 건 낭비다. 코드가 컴파일조차 안 된다면 AC 충족 여부를 따지기 전에 먼저 고쳐야 한다. Short-Circuit으로 불필요한 LLM 호출을 줄인다.

**왜 overallScore와 goalAlignment를 별도로 측정하는가?**
AC를 모두 충족해도 Spec의 핵심 목표와 어긋날 수 있다. 예를 들어 "로그인 기능 구현"의 모든 AC를 통과했지만 실제 구현이 OAuth 대신 기본 인증을 사용해 보안 요구사항을 만족하지 못하는 경우다. goalAlignment는 이런 의미적 드리프트를 잡는다.

**Structural 실패 시 왜 바로 Evolve로 가는가?**
Structural 실패 원인은 명확하다 — 코드 오류, 누락된 의존성, 잘못된 명령어. LLM의 추가 평가 없이 오류 메시지만으로 Fix 컨텍스트를 만들 수 있다. Contextual을 돌리면 불완전한 결과에 대해 잘못된 신뢰를 줄 수 있다.
