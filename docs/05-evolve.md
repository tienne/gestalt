...
이 단계를 마치면 Evaluate 미달 원인을 진단하고, Spec 수정 또는 관점 전환으로 실행 결과를 성공 기준까지 끌어올릴 수 있어요.

Evaluate가 성공 임계값에 미달하면 Evolve가 시작돼요. 실패 원인에 따라 세 가지 흐름 중 하나로 분기해요.
...
// Step 3: 영향받은 태스크 재실행 (allTasksCompleted === true까지 반복)
ges_execute({
...
// 재실행: evolve_re_execute 재사용
// Re-evaluate: evaluate 재사용
...
각 Evolution 세대마다 Spec 스냅샷과 delta를 기록해요.
...
두 패턴은 원인이 달라요. Oscillation(점수 반복)은 Spec 안에 서로 충돌하는 요구사항이 있다는 신호예요. 이 경우 단순화(Simplicity)가 처방이에요. Stagnation(점수 정체)은 빠진 요소가 있거나 방향이 틀렸다는 신호예요. 이 경우 재해석(Multistability)이나 빈틈 채우기(Reification)가 처방이에요.

**왜 Caller 종료를 명시적으로 지원하나요?**

Passthrough 모드에서 Caller가 전체 흐름을 제어해요. Caller(Claude Code 등)가 "이제 충분해요" 또는 "사용자가 요청을 취소했어요"라고 판단할 때 강제 종료할 수 있어야 해요.
