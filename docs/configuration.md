...
| `importPattern` | string (선택) | 이 소스를 쓰는 import를 알아보는 정규식. `design-check`가 시스템 안팎을 가를 때 쓴다. 비우면 `ref`에서 추론한다 |

**선언한 것만 읽는다.** 붙어 있는 MCP 서버를 훑어 관련 있어 보이는 걸 골라 쓰지 않는다. 무엇을 근거로 삼았는지 불투명해진다. 이름만 비슷한 엉뚱한 걸 물 수 있다. 선언이 없으면 스킬은 이 단계를 통째로 건너뛴다.

**안 적으면 `ref`에서 알아낸다.** `ref`가 가리킨 경로에서 위로 올라가며 가장 가까운 `package.json`의 이름을 읽어 패턴을 만든다. 붙어 있는 걸 훑어 고르는 것과 다르다 — 이미 `ref`로 지목한 소스가 어느 패키지인지만 읽으므로 근거는 여전히 선언이다. 어떻게 알아냈는지는 검사 결과의 "기준"에 적힌다.

올라가다 검사 대상 레포의 루트에 닿으면 멈춘다. 거기까지 가면 그 레포 자기 이름을 집어 자기 소스를 디자인 시스템으로 보게 된다. 그때는 추론하지 않고 사람이 적게 둔다.
