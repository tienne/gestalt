...
선언 하나가 잘못되면 zod가 `ruleSources` 배열을 통째로 기본값(빈 배열)으로 되돌린다. 그 상태를 그냥 두면 스킬이 "선언 안 한 레포"와 구분할 수 없어, **오타 하나 때문에 `stop`으로 걸어둔 검사가 조용히 안 돈다**. 그래서 왜 비었는지를 여기 함께 싣고 스킬이 멈춘다.
