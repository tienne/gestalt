...
서로 리뷰하게 했더니, 게이트가 깨진 채 올라온 PR, 뮤테이션에도 안 죽는 빈 테스트,
...
이벤트 소싱이다. 상태를 따로 저장하지 않고 이벤트를 접어 만든다(`src/local-pr/repository.ts`).
...
리뷰어의 워크트리는 자기 브랜치에 올라타 있어서 PR 코드가 거기 없다. 떼어낸다.
...
`--detach`로 뗀다. 브랜치를 잡으면 그 브랜치를 이미 올라탄 워크트리와 부딪힌다. 같은 PR을
두 번 불러도 워크트리는 하나이고 그 안의 변경은 살아남는다.
...
`absent`가 0인 이유는 지울 자리가 없는 게 정리의 목표가 이미 이뤄진 상태여서다. 4로 주면
...
- **지금 자리가 base를 올라타고 있으면** 그 자리에서 곧장 합친다.
- **아무 워크트리도 base를 안 잡고 있으면** 임시 워크트리에서 합치고 브랜치 ref만 옮긴다.
  워커가 자기 워크트리에 그대로 있는 채로 머지할 수 있다.
- **다른 워크트리가 base를 잡고 있으면** 거부한다. 그쪽은 파일이 옛 상태인데 HEAD만
