...
에이전트가 stdout을 파싱하지 않고도 분기하게 만든 값이다.
...
`pr.edited`는 상태만 접는 경로(`foldStatus`)가 아예 안 읽는다. 거기 넣는 순간 판정이
...
정리 결과는 `status`로 분기한다. 산문 `reason`을 부분 문자열로 긁지 않는다.
...
127.0.0.1에만 붙는 읽기 전용 뷰다. 코멘트 작성과 판정은 CLI와 `ges_pr`에만 뒀다 — 기록을
남기는 경로를 웹까지 벌리면 하나가 뒤처진다.

**서버 하나가 여러 레포를 보여준다.** `pr serve`를 한 번이라도 돌린 레포는 `~/.gestalt/repos.json`에
등록되고 그다음부터는 어느 레포에서 `pr serve`를 치든 전부 뜬다. `/`는 지금 자리의 레포로
보내고 위쪽 줄에서 다른 레포로 옮겨 간다.

등록은 `pr serve`만 한다. `pr create`는 목록을 안 늘린다 — 그 경로는 MCP `ges_pr`의
`repoRoot`로 이어져 있어서 등록하면 도구 호출 한 번이 인증 없는 서버가 열어 줄 레포를
...
`pr` 스킬은 GitHub 전용이라 로컬 경로를 안 탄다. 로컬이라고 밝히면 만들지 않고 `local-pr`로
넘긴다. 대신 원격 PR을 만들기 전에 이 브랜치의 안 끝난 로컬 PR을 찾아 사용자에게 알린다.

`review`와 `review-reply`는 양쪽을 다 탄다. `--local`을 붙이거나 로컬 PR id를 주면
