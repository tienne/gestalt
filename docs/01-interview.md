# 1단계: Interview — 요구사항의 명확도를 측정하기

AI에게 "사용자 인증 시스템 만들어줘"라고 하면, AI는 당신이 원하는 게 JWT인지 세션인지 알 수 없어요. OAuth를 지원해야 하는지, 비밀번호 찾기는 필요한지도 마찬가지예요. Interview 단계는 이런 불확실함을 Q&A로 수치화해서 스펙 생성 전에 요구사항을 충분히 명확하게 만들어줘요.

---

## Interview는 왜 필요한가요?

요구사항이 모호한 채로 스펙을 만들면 실행 단계에서 충돌이 생겨요. Interview는 하나의 토픽(topic)으로 시작해서 여러 라운드의 Q&A를 거치며 **해상도(Resolution Score)가 0.8 이상**이 될 때까지 질문을 이어가요. 각 라운드에서 어떤 질문을 해야 하는지는 게슈탈트 5원리가 동적으로 결정해요.
...
## 해상도(Resolution Score)는 어떻게 계산되나요?
...
- `overall ≥ 0.8` → `isReady = true` → 인터뷰를 종료할 수 있어요
- `overall < 0.8` → 다음 라운드를 계속해요

### Continuity 페널티

모순이 감지되면 overall에서 페널티가 차감돼요. 모순이 해소되지 않으면 해상도가 0.8에 도달하기 어렵게 설계했어요.
...
3. ges_interview({ action: "respond", generatedQuestion: "...", response: "...", resolutionScore?: {...} })
...
모순이 있는 채로 스펙을 생성하면 실행 단계에서 충돌이 발생해요. Continuity 페널티가 해상도를 낮게 유지해서 인터뷰가 강제로 계속되도록 설계했어요.
