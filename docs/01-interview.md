...
AI에게 "사용자 인증 시스템 만들어줘"라고 하면, AI는 당신이 원하는 게 JWT인지 세션인지 알 수 없어요. OAuth를 지원해야 하는지, 비밀번호 찾기는 필요한지도 마찬가지예요. Interview 단계는 이 모호함을 Q&A로 수치화해서 스펙 생성 전에 요구사항을 충분히 명확하게 만들어줘요.
